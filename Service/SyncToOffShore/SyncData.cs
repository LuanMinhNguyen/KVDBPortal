...
namespace SyncToOffShore
{
    public partial class SyncData : ServiceBase
    {
        ...
        private IntergrateParamConfigService configService;
        private ExportDataChangeHistoryService exportHistoryService;
        private ImportDataChangeHistoryService importHistoryService;
        private WaitingSyncDataService syncWaitingService;

        private  TitleService titleService;

        private  ScopeProjectService projectService;

        private  MenuService menuService;

        private MRTypeService mrTypeService;

        private  UserService userService;

        private  PriorityLevelService priorityLevelService;

        private  MaterialRequisitionDetailService mrDetailService;

        private  MaterialRequisitionCheckListDetailService mrChecklistDetailService;

        private  MaterialRequisitionService mrService;

        private  MaterialRequisitionAttachFileService mrAttachFileService;

        private  MaterialRequisitionCheckListDefineService mrCheckListDefineService;

        private  MaterialRequisitionCommentService mrCommentService;

        private  NumberManagementService numberManagementService;

        private  ObjectAssignedUserService objAssignedUserService;

        private  ObjectAssignedWorkflowService objAssignedWorkflowService;

        private  RoleService roleService;

        private  PermissionService permissionService;

        private TrackingMorningCallService trackingMorningCallService;

        private TrackingMorningCallAttachFileService trackingMorningCallAttachFileService;

        private TrackingMorningCallCommentService trackingMorningCallCommentService;

        private TrackingBreakdownReportService trackingBreakdownReportService;

        private TrackingBreakdownReportAttachFileService trackingBreakdownReportAttachFileService;

        private TrackingBreakdownReportCommentService trackingBreakdownReportCommentService;

        private TrackingECRService trackingECRService;

        private TrackingECRAttachFileService trackingECRAttachFileService;

        private TrackingECRCommentService trackingEcrCommentService;

        private TrackingMOCService trackingMOCService;

        private TrackingMOCAttachFileService trackingMOCAttachFileService;

        private TrackingMOCCommentService trackingMocCommentService;

        private TrackingProcedureService trackingProcedureService;

        private TrackingProcedureAttachFileService trackingProcedureAttachFileService;

        private TrackingPunchService trackingPunchService;

        private TrackingPunchAttachFileService trackingPunchAttachFileService;

        private TrackingSailService trackingSailService;

        private TrackingSailAttachFileService trackingSailAttachFileService;

        private TrackingShutdownReportService trackingShutdownReportService;

        private TrackingShutdownReportAttachFileService trackingShutdownReportAttachFileService;

        private TrackingShutdownReportCommentService trackingShutdownReportCommentService;

        private TrackingWCRService trackingWCRService;

        private TrackingWCRAttachFileService trackingWCRAttachFileService;

        private WorkflowService wfService;

        private WorkflowDetailService wfDetailService;

        private WorkflowStepService wfStepService;

        private WorkRequestService workRequestService;

        private WorkRequestAttachFileService workRequestAttachFileService;

        private WorkRequestCommentService workRequestCommentService;

        private ImportEmailDownloadedService importEmailDownloadedService;

        private TrackingGeneralWorkingService trackingGeneralWorkingService;

        private TrackingGeneralWorkingAttachFileService trackingGeneralWorkingAttachFileService;
        ...
        private PerformanceReviewAttachFileService prAttachFileService;
        ...
        protected override void OnStart(string[] args)
        {
            try
            {
                ...
                this.EventLog.WriteEntry("WMS Sync data service started");
        ...
        {
            timerSync.Dispose();
            this.EventLog.WriteEntry("WMS Sync data service stoped");
        }
        ...
        private void Timer_Fired(object sender, EventArgs e)
        {
            ...
            {
                timerSync.Stop();
                configService = new IntergrateParamConfigService();
                exportScheduleService = new ExportScheduleService();

                exportHistoryService = new ExportDataChangeHistoryService();
                importHistoryService = new ImportDataChangeHistoryService();
                syncWaitingService = new WaitingSyncDataService();

                importEmailDownloadedService = new ImportEmailDownloadedService();
                ...
                if (configObj != null)
                {
                    ...
                    if (dataChangedList.Count > 0)
                    {
                        var exportTimeList = exportScheduleService.GetAll();
                        foreach (var exportSchedule in exportTimeList)
                        {
                            if (exportSchedule.CurrentDay == null)
                            {
                                exportSchedule.CurrentDay = DateTime.Now;
                                exportSchedule.IsActive = true;
                                this.exportScheduleService.Update(exportSchedule);
                            }
                            else
                            {
                                if (exportSchedule.CurrentDay.Value.ToString("dd/MM/yyyy") !=
                            ...
                            if (exportSchedule.IsActive.GetValueOrDefault()
                                && exportSchedule.ExportTime.GetValueOrDefault().Hour == currentTime.Hour
                                && exportSchedule.ExportTime.GetValueOrDefault().Minute <= currentTime.Minute)
                            {
                                // Do Export Data change file
                                if (!string.IsNullOrEmpty(configObj.Sync_ExportFolder) &&
                                    Directory.Exists(configObj.Sync_ExportFolder))
                                {
                                    var fileName = configObj.Sync_EmailName + "_Data Changed_" +
                                                   DateTime.Now.ToString("dd-MM-yyyy HHmmss_") + (this.exportHistoryService.GetCount() + 1) + ".sql";
                                    var filePath = configObj.Sync_ExportFolder + "\\" + fileName;

                                    // Trascking Export History
                                    exportSchedule.IsActive = false;
                                    this.exportScheduleService.Update(exportSchedule);
                                    ...
                                    try
                                    {
                                        this.ExportDataFile(filePath, dataChangedList);
                                    ...
                                    if (configObj.IsAutoGetSend.GetValueOrDefault() && !string.IsNullOrEmpty(configObj.Sync_EmailSendExport))
                                    {
                                        try
                                        {
                                            ...
                                            message.Subject = "New data changed of from " + configObj.Sync_EmailName;
                                            foreach (var email in configObj.Sync_EmailSendExport.Split(';').Where(t => !string.IsNullOrEmpty(t)))
                                            {
                                            ...
                                            exportHistory.SendMailComplete = true;
                                            this.exportHistoryService.Update(exportHistory);
                                        }
                                        catch (Exception ex)
                                        {
                                            exportHistory.SendMailComplete = false;
                                            exportHistory.ErrorMess += "Send mail error: " + ex.Message + Environment.NewLine;
                                            this.exportHistoryService.Update(exportHistory);
                                        }
                                    ...
                                    this.EventLog.WriteEntry("Export completed data change file: " + filePath);
                                }
                                else
                                {
                                    this.EventLog.WriteEntry("Can't find config param for Export data folder.");
                                }
                            }
                        }
                    }
                    ...
                    // Get new email
                    if (configObj.IsAutoGetSend.GetValueOrDefault())
                    {
                        var seeEmailUid = importEmailDownloadedService.GetAll().Select(t => t.MessUid).ToList();
                        // Fetch all the current uids seen
                        var pop3Client = new Pop3Client();
                        pop3Client.Connect(configObj.Sync_PopServer, Convert.ToInt32(configObj.Sync_PopPort), true);
                        pop3Client.Authenticate(configObj.Sync_DefaultEmail, configObj.Sync_EmailPwd, AuthenticationMethod.UsernameAndPassword);
                        
                        List<string> uids = pop3Client.GetMessageUids();

                        if (!uids.All(t => seeEmailUid.Contains(t)))
                        {
                            for (int i = 0; i < uids.Count; i++)
                            {
                                string currentUidOnServer = uids[i];
                                if (!seeEmailUid.Contains(currentUidOnServer))
                                {
                                    var unseenMessage = pop3Client.GetMessage(i + 1);
                                    seeEmailUid.Add(currentUidOnServer);

                                    // Tracking seen mess history
                                    var importEmailDownloaded = new ImportEmailDownloaded()
                                    {
                                        MessUid = currentUidOnServer
                                    };
                                    importEmailDownloadedService.Insert(importEmailDownloaded);
                                    // ------------------------------------------------------------
                                    ...
                                    {
                                        ByteArrayToFile(configObj.Sync_ImportFolder +  "\\" + messagePart.FileName, messagePart.Body);
                                    }
                                    // ------------------------------------------------------------
                                }
                            }
                    ...
                    // Import data change
                    if (!string.IsNullOrEmpty(configObj.Sync_ImportFolder)
                        && Directory.Exists(configObj.Sync_ImportFolder)
                        && !string.IsNullOrEmpty(configObj.Sync_ImportConnStr))
                    {
                        var dirInfo = new DirectoryInfo(configObj.Sync_ImportFolder);
                        var importHistory = importHistoryService.GetAll().Select(t => t.FileName);
                        var importFileList = dirInfo.GetFiles().Where(t=> t.Name.Contains(".sql")).Select(t => t.Name);

                        var newFile = importFileList.Where(t => !importHistory.Contains(t)).ToList();
                        conn = new SqlConnection(configObj.Sync_ImportConnStr);
                        conn.Open();
                        foreach (var fileName in newFile)
                        {
                            var filePath = configObj.Sync_ImportFolder + "\\" + fileName;
                            ...
                            while ((line = file.ReadLine()) != null)
                            {
                                if (!string.IsNullOrEmpty(line))
                                {
                                    ...
                                    {
                                        importItem.IsComplete = false;
                                        importItem.ErrorMess += ex.Message + Environment.NewLine;
                    ...
                    {
                        this.EventLog.WriteEntry("Can't find config param for Import folder or Connect string to database.");
                    }
                    // ---------------------------------------------------------------------------------------------------------
                }
                else
                {
                    this.EventLog.WriteEntry("Can't find config param for Integrate and Sync Data.");
                }
            }
            catch (Exception ex)
            {
        ...
        private void ExportDataFile(string filePath, List<WaitingSyncData> dataChangedList)
        {
            try
            {
                ...
                this.trackingBreakdownReportAttachFileService = new TrackingBreakdownReportAttachFileService();
                this.trackingECRService = new TrackingECRService();
                this.trackingECRAttachFileService = new TrackingECRAttachFileService();
                this.trackingGeneralWorkingService = new TrackingGeneralWorkingService();
                this.trackingGeneralWorkingAttachFileService = new TrackingGeneralWorkingAttachFileService();
                this.trackingMOCService = new TrackingMOCService();
                this.trackingMOCAttachFileService = new TrackingMOCAttachFileService();
                this.trackingMorningCallAttachFileService = new TrackingMorningCallAttachFileService();
                this.trackingMorningCallService = new TrackingMorningCallService();
                this.trackingProcedureService = new TrackingProcedureService();
                this.trackingProcedureAttachFileService = new TrackingProcedureAttachFileService();
                this.trackingPunchAttachFileService = new TrackingPunchAttachFileService();
                this.trackingPunchService = new TrackingPunchService();
                this.trackingSailAttachFileService = new TrackingSailAttachFileService();
                this.trackingSailService = new TrackingSailService();
                this.trackingShutdownReportAttachFileService = new TrackingShutdownReportAttachFileService();
                this.trackingShutdownReportService = new TrackingShutdownReportService();
                this.trackingWCRService = new TrackingWCRService();
                this.trackingWCRAttachFileService = new TrackingWCRAttachFileService();
                this.workRequestService = new WorkRequestService();
                this.workRequestAttachFileService = new WorkRequestAttachFileService();
                this.workRequestCommentService = new WorkRequestCommentService();
                ...
                this.prAttachFileService = new PerformanceReviewAttachFileService();
        ...
        private string GetObjWorkflowChanged(WaitingSyncData changeInfo, Workflow obj)
        {
            var cmd = string.Empty;
            switch (changeInfo.ActionTypeID)
            {
