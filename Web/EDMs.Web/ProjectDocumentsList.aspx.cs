...
namespace EDMs.Web
{
	...
	/// Class customer
	/// </summary>
	public partial class ProjectDocumentsList : Page
	{
		...
		private readonly DocumentCodeServices documnetCodeSErvie = new DocumentCodeServices();
		...
		protected void RadAjaxManager1_AjaxRequest(object sender, AjaxRequestEventArgs e)
		{
			...
			else if (e.Argument == "ExportMasterList")
			{
				...
				var unitCodeList = this.unitService.GetAll();
				var docTypeList = this.documentTypeService.GetAll().Where(t => t.ParentId != null).ToList();
				var kksCodeList = this.kksService.GetAll();
				var disciplineList = this.disciplineService.GetAll();
				var organisationList = this.organizationcodeService.GetAll();
				var revisionStatustlist = this.revisionStatusService.GetAll();
				var documentcodelist = this.documnetCodeSErvie.GetAll();
				var revisionSchemaList = this.revisionSchemaService.GetAll();
				var subsystemList = this.systemCodeService.GetAll().Where(t => t.ParentId != null).ToList();
				...
				{
					readMeSheet.Cells["E" + (7 + i)].PutValue(confdentialitylist[i].Code);
					readMeSheet.Cells["F" + (7 + i)].PutValue(confdentialitylist[i].Description);
				}

				for (int i = 0; i < unitCodeList.Count; i++)
				{
					readMeSheet.Cells["H" + (7 + i)].PutValue(unitCodeList[i].Code);
					readMeSheet.Cells["I" + (7 + i)].PutValue(unitCodeList[i].Description);
				}

				for (int i = 0; i < docTypeList.Count; i++)
				{
					readMeSheet.Cells["K" + (7 + i)].PutValue(docTypeList[i].Code);
					readMeSheet.Cells["L" + (7 + i)].PutValue(docTypeList[i].Description);
				}

				for (int i = 0; i < kksCodeList.Count; i++)
				{
					readMeSheet.Cells["N" + (7 + i)].PutValue(kksCodeList[i].Code);
					readMeSheet.Cells["O" + (7 + i)].PutValue(kksCodeList[i].Description);
				}

				for (int i = 0; i < disciplineList.Count; i++)
				{
					readMeSheet.Cells["Q" + (7 + i)].PutValue(disciplineList[i].Code);
					readMeSheet.Cells["R" + (7 + i)].PutValue(disciplineList[i].Description);
				}

				for (int i = 0; i < organisationList.Count; i++)
				{
					readMeSheet.Cells["T" + (7 + i)].PutValue(organisationList[i].Code);
					readMeSheet.Cells["U" + (7 + i)].PutValue(organisationList[i].Description);
				}

				for (int i = 0; i < revisionSchemaList.Count; i++)
				{
					readMeSheet.Cells["W" + (7 + i)].PutValue(revisionSchemaList[i].Code);
					readMeSheet.Cells["X" + (7 + i)].PutValue(revisionSchemaList[i].Description);
				}

				for (int i = 0; i < revisionStatustlist.Count; i++)
				{
					readMeSheet.Cells["Z" + (7 + i)].PutValue(revisionStatustlist[i].Code);
					readMeSheet.Cells["AA" + (7 + i)].PutValue(revisionStatustlist[i].Description);
				}

				for (int i = 0; i < documentcodelist.Count; i++)
				{
					readMeSheet.Cells["AC" + (7 + i)].PutValue(documentcodelist[i].Code);
					readMeSheet.Cells["AD" + (7 + i)].PutValue(documentcodelist[i].Description);
				}

				for (int i = 0; i < subsystemList.Count; i++)
				{
					readMeSheet.Cells["AF" + (7 + i)].PutValue(subsystemList[i].Code);
					readMeSheet.Cells["AG" + (7 + i)].PutValue(subsystemList[i].Description);
				}
				...
				var filename = "PECC2_EMDR Template_" + DateTime.Now.ToString("ddmmyyyy") + ".xls";
		...
		protected void grdDocument_OnNeedDataSource(object source, GridNeedDataSourceEventArgs e)
		{
			...
			var txtSearch = (System.Web.UI.WebControls.TextBox)this.CustomerMenu.Items[5].FindControl("txtSearch");
			var Ckbshowall= (System.Web.UI.WebControls.CheckBox)this.CustomerMenu.Items[4].FindControl("ckbShowAll");
			var docList = new List<PECC2Documents>();
			var alldocOfProject = this._PECC2documentService.GetByProjectCode(projectId, UserSession.Current.User.ConfidentialId.GetValueOrDefault(), Ckbshowall.Checked);
			...
			if (this.rtvTreeNode.SelectedNode != null)
			{
				...
				if (tagnode.Contains("DocType"))
				{
					tagnode = tagnode.Split('_')[0];
					selectedNodesId =
						this.rtvTreeNode.SelectedNode.GetAllNodes().Where(t => t.Target.Split('_')[0] == tagnode).Select(t => Convert.ToInt32(t.Value)).ToList();
				}
				...
				{
					if(tagnode== "Plant") {
						docList = alldocOfProject.ToList();
					}
				   else  if (tagnode == "Area")
					{
						docList = alldocOfProject.Where(t => selectedNodesId.Contains(t.AreaId.GetValueOrDefault())).ToList();
					}
					else if (tagnode == "Unit")
					{
						docList = alldocOfProject.Where(t => selectedNodesId.Contains(t.UnitId.GetValueOrDefault())).ToList();
					}
					else if (tagnode == "Discipline")
					{
						docList = alldocOfProject.Where(t => selectedNodesId.Contains(t.DisciplineId.GetValueOrDefault())).ToList();
					}
					else if (tagnode == "Organization")
					{
						docList = alldocOfProject.Where(t => selectedNodesId.Contains(t.OriginatingOrganisationId.GetValueOrDefault())).ToList();
					}
					else if (tagnode.Contains("DocType"))
					{
						//var categoryId = Convert.ToInt32(tagnode.Split('_')[1]);
					   // docList = alldocOfProject.Where(t => selectedNodesId.Contains(t.DocTypeId.GetValueOrDefault())
					  //  && t.CategoryId == categoryId).ToList();
						docList = alldocOfProject.Where(t => selectedNodesId.Contains(t.DocTypeId.GetValueOrDefault())).ToList();
					}else if(tagnode== "Category")
					{
						docList = alldocOfProject.Where(t => selectedNodesId.Contains(t.CategoryId.GetValueOrDefault())).ToList();
					}
				}
			}
			else
			{
				docList = alldocOfProject;
			}
			if (!string.IsNullOrEmpty(txtSearch.Text))
			{
				var listkey = txtSearch.Text.ToLower().Split(' ').ToArray();
				docList=docList.Where(t=> listkey.All(k =>(t.DocNo.ToLower()+" "+ t.DocTitle.ToLower()).Contains(k))).ToList();
			   
			}

			this.grdDocument.DataSource = docList.OrderBy(t => t.DocNo).ThenBy(t=> t.CreatedDate);
		}
		...
		protected void grdDocument_DeleteCommand(object sender, GridCommandEventArgs e)
		{
			...
			if (docObj != null)
			{
				...
				{
					var listRelateDoc = this._PECC2documentService.GetAllRelatedDocument(docObj.ParentId.GetValueOrDefault());
					if (listRelateDoc != null)
					{
						foreach (var objDoc in listRelateDoc)
						{
		...
		private void LoadObjectTree()
		{
			...
			//this.lblCategoryId.Value = this.ddlCategory.SelectedValue;
			// add eoject code
			...
			foreach(var item in plantlist)
			{
				...
				node.Target = "Plant";
				//Add node Area
				var arealist = this.areaService.GetAllPlant(item.ID);
				foreach( var area in arealist)
				{
				   var nodearea = new RadTreeNode();
					...
					//Add node Unit
					var unitlist = this.unitService.GetAllArea(area.ID);
			...
			foreach(var type  in documenttypelist.Where(t=> t.ParentId==null))
			{
				...
				node.Target = "DocType_0";
				foreach (var types in documenttypelist.Where(t => t.ParentId == type.ID).OrderBy(t => t.Code))
				{
					...
					nodes.Target = "DocType_"+ types.CategoryIds;
			...
			// Add node Discipline
			var disciplinelist = this.disciplineService.GetAll().OrderBy(t => t.Code).ThenBy(t => t.Description);
			...
			var originationlist = this.organizationcodeService.GetAll();
			root = new RadTreeNode();
			...
			//category
			var ListCategory = new Dictionary<string, string>()
					{
						{ "1", "1. GENERAL" },
						{ "2", "2. ENGINEERING" },
						{ "3", "3. CONSTRUCTION" },
			...
			root = new RadTreeNode();
			root.Text = "Category";
			root.ImageUrl = "~/Images/category.png";
			root.Value = "-1";
			...
			foreach (var item in ListCategory)
			{
				...
				node.Value = item.Key;
				node.ImageUrl = "~/Images/category.png";
				node.Target = "Category";
				root.Nodes.Add(node);
			}
			this.rtvTreeNode.Nodes.Add(root);
		}
		...
		protected void rtvTreeNode_NodeClick(object sender, RadTreeNodeEventArgs e)
		{
			...
			if (tagnode.Contains("DocType"))
			{
				var categotyId = Convert.ToInt32(tagnode.Split('_')[1]);
			   // this.InitGridColumnView(categotyId);
			}
		...
		private void ExportCMDRReport()
		{
			...
			var engSheet = workbook.Worksheets[0];
			var vendorSheet = workbook.Worksheets[0];
			var csSheet = workbook.Worksheets[0];
			var momSheet = workbook.Worksheets[0];

			var projectName = this.ddlProject.SelectedItem != null
				? this.ddlProject.SelectedItem.Text.Replace(",", "-")
				: string.Empty;
			engSheet.Cells["E1"].PutValue(this.ddlProject.Text);
			engSheet.Cells["H2"].PutValue(DateTime.Now.ToString("dd-MMM-yy"));

			//vendorSheet.Cells["E1"].PutValue(this.ddlProject.Text);
			//vendorSheet.Cells["I2"].PutValue(DateTime.Now.ToString("dd-MMM-yy"));

			//csSheet.Cells["E1"].PutValue(this.ddlProject.Text);
			//csSheet.Cells["I2"].PutValue(DateTime.Now.ToString("dd-MMM-yy"));

			//momSheet.Cells["E1"].PutValue(this.ddlProject.Text);
			//momSheet.Cells["I2"].PutValue(DateTime.Now.ToString("dd-MMM-yy"));
			...
			var fullDocList = this._PECC2documentService.GetByProjectCode(projectId,
				UserSession.Current.User.ConfidentialId.GetValueOrDefault(),true);
			var engDocList = fullDocList.ToList();
			//var vendorDocList = fullDocList.Where(t => t.CategoryId == 2).ToList();
			//var csDocList = fullDocList.Where(t => t.CategoryId == 3).ToList();
			//var momDocList = fullDocList.Where(t => t.CategoryId == 4).ToList();
			...
			dtEngDoc.Columns.AddRange(new[]
			{
				...
				new DataColumn("SheetNo", typeof (String)),
				new DataColumn("PlanedDate", typeof (String)),
				new DataColumn("ActualDate", typeof (String)),
				 new DataColumn("DocActionCode", typeof (String)),
				 new DataColumn("DocReviewStatus", typeof (String)),
                new DataColumn("Remark", typeof (String)),
            });

			for (int i = 0; i < engDocList.Count; i++)
            {
               
                var dataRow = dtEngDoc.NewRow();
				dataRow["DocId"] = engDocList[i].ID.ToString();
				dataRow["NoIndex"] = (i + 1).ToString();
				dataRow["DocNo"] = engDocList[i].DocNo;
				dataRow["DocTitle"] = engDocList[i].DocTitle;
				dataRow["Revision"] = engDocList[i].Revision;
				dataRow["DocType"] = engDocList[i].DocTypeCode;
				dataRow["Category"] = engDocList[i].CategoryName;
				dataRow["IssuedDate"] = engDocList[i].IncomingTransId != null ? this._PECC2TransmittalService.GetById(engDocList[i].IncomingTransId.GetValueOrDefault())?.IssuedDate.GetValueOrDefault().ToString("dd-MMM-yyyy") : String.Empty;
				dataRow["Area"] = engDocList[i].AreaCode;
				dataRow["Unit"] = engDocList[i].UnitCode;
				dataRow["KKSCode"] = engDocList[i].KKSCode;
				dataRow["TrainNo"] = engDocList[i].TrainNo;
				dataRow["Discipline"] = engDocList[i].DisciplineCode;
				dataRow["SheetNo"] = engDocList[i].SheetNo;
				dataRow["PlanedDate"] = engDocList[i].IncomingTransId != null ? this._PECC2TransmittalService.GetById(engDocList[i].IncomingTransId.GetValueOrDefault())?.DueDate.GetValueOrDefault().ToString("dd-MMM-yyyy") : String.Empty;
                dataRow["ActualDate"] = engDocList[i].OutgoingTransId != null ? this._PECC2TransmittalService.GetById(engDocList[i].OutgoingTransId.GetValueOrDefault())?.IssuedDate.GetValueOrDefault().ToString("dd-MMM-yyyy") : String.Empty;
				dataRow["DocActionCode"] = engDocList[i].DocActionCode;
				dataRow["DocReviewStatus"] = engDocList[i].DocReviewStatusCode;
                dataRow["Remark"] = engDocList[i].Remarks;

                dtEngDoc.Rows.Add(dataRow);
			}
            engSheet.Cells["A7"].PutValue(dtEngDoc.Rows.Count);

            engSheet.Cells.ImportDataTable(dtEngDoc, false, 5, 1, dtEngDoc.Rows.Count, dtEngDoc.Columns.Count, true);
   //         #region Hiden
   //         var dtVendorDoc = new DataTable();
			//dtVendorDoc.Columns.AddRange(new[]
			//{
			//	new DataColumn("DocId", typeof (String)),
			//	new DataColumn("NoIndex", typeof (String)),
			//	new DataColumn("DocNo", typeof (String)),
			...
			//	new DataColumn("Confidentiality", typeof (String)),
			//	new DataColumn("DocType", typeof (String)),
			//	new DataColumn("OriginatingOrganisation", typeof (String)),
			//	new DataColumn("ReceivingOrganisation", typeof (String)),
			//	new DataColumn("GroupCode", typeof (String)),
			//	new DataColumn("OriginalDocNo", typeof (String)),
			//	new DataColumn("IssuedDate", typeof (String)),
			//	new DataColumn("Area", typeof (String)),
			//	new DataColumn("Unit", typeof (String)),
			//	new DataColumn("System", typeof (String)),
			//	new DataColumn("SubSystem", typeof (String)),
			//	new DataColumn("PlanedDate", typeof (String)),
			//	new DataColumn("ActualDate", typeof (String)),
			//	new DataColumn("ManHour", typeof (String)),
			//	new DataColumn("Remark", typeof (String)),
			//	new DataColumn("RevSchema", typeof (String)),
			//	new DataColumn("MinorRev", typeof (String)),
			//	new DataColumn("RevStatus", typeof (String)),
			...
			//for (int i = 0; i < vendorDocList.Count; i++)
			//{
			//	var dataRow = dtVendorDoc.NewRow();
			//	dataRow["DocId"] = vendorDocList[i].ID.ToString();
			//	dataRow["NoIndex"] = (i + 1).ToString();
			//	dataRow["DocNo"] = vendorDocList[i].DocNo;
			//	dataRow["DocTitle"] = vendorDocList[i].DocTitle;
			//	dataRow["Revision"] = vendorDocList[i].Revision;
			//	dataRow["Confidentiality"] = vendorDocList[i].ConfidentialityName;
			//	dataRow["DocType"] = vendorDocList[i].DocTypeCode;
			//	dataRow["OriginatingOrganisation"] = vendorDocList[i].OriginatingOrganisationName;
			//	dataRow["ReceivingOrganisation"] = vendorDocList[i].ReceivingOrganisationName;
			//	dataRow["GroupCode"] = vendorDocList[i].GroupCode;
			//	dataRow["OriginalDocNo"] = vendorDocList[i].OriginalDocumentNumber;
			//	dataRow["IssuedDate"] = vendorDocList[i].Date != null ? vendorDocList[i].Date.GetValueOrDefault().ToString("dd-MMM-yyyy") : String.Empty;
			//	dataRow["Area"] = vendorDocList[i].AreaCode;
			//	dataRow["Unit"] = vendorDocList[i].UnitCode;
			//	dataRow["System"] = vendorDocList[i].SystemCode;
			//	dataRow["SubSystem"] = vendorDocList[i].SubsystemCode;
			//	dataRow["PlanedDate"] = vendorDocList[i].PlannedDate != null ? vendorDocList[i].PlannedDate.GetValueOrDefault().ToString("dd-MMM-yyyy") : String.Empty;
			//	dataRow["ActualDate"] = vendorDocList[i].ActualDate != null ? vendorDocList[i].ActualDate.GetValueOrDefault().ToString("dd-MMM-yyyy") : String.Empty;
			//	dataRow["ManHour"] = vendorDocList[i].ManHour;
			//	dataRow["Remark"] = vendorDocList[i].Remarks;
			//	dataRow["RevSchema"] = vendorDocList[i].RevisionSchemaName;
			//	dataRow["MinorRev"] = vendorDocList[i].MinorRev;
			//	dataRow["RevStatus"] = vendorDocList[i].RevStatusName;
			//	dataRow["RevRemark"] = vendorDocList[i].RevRemarks;
			//	dataRow["DocActionCode"] = vendorDocList[i].DocActionCode;
			//	dataRow["DocReviewStatus"] = vendorDocList[i].DocReviewStatusCode;

			//	dtVendorDoc.Rows.Add(dataRow);
			//}
			//vendorSheet.Cells.ImportDataTable(dtVendorDoc, false, 6, 1, dtVendorDoc.Rows.Count, dtVendorDoc.Columns.Count, true);

			//var dtCSDoc = new DataTable();
			//dtCSDoc.Columns.AddRange(new[]
			//{
			//	new DataColumn("DocId", typeof (String)),
			//	new DataColumn("NoIndex", typeof (String)),
			//	new DataColumn("DocNo", typeof (String)),
			...
			//	new DataColumn("GroupCode", typeof (String)),
			//	new DataColumn("CarbonCopy", typeof (String)),
			//	new DataColumn("RelatedCS", typeof (String)),
			//	new DataColumn("NeedReply", typeof (String)),
			//	new DataColumn("Description", typeof (String)),
			//	new DataColumn("Treatment", typeof (String)),
			//	new DataColumn("ProposedBy", typeof (String)),
			//	new DataColumn("ProposedDate", typeof (String)),
			//	new DataColumn("ReviewedBy", typeof (String)),
			//	new DataColumn("ReviewedDate", typeof (String)),
			//	new DataColumn("ApprovedBy", typeof (String)),
			//	new DataColumn("ApprovedDate", typeof (String)),
			//	new DataColumn("IssuedDateFROM", typeof (String)),
			//	new DataColumn("ReceivedDateTO", typeof (String)),
			//	new DataColumn("ReceivedDateCC", typeof (String)),
			//});

			//for (int i = 0; i < csDocList.Count; i++)
			//{
			//	var dataRow = dtCSDoc.NewRow();
			//	dataRow["DocId"] = csDocList[i].ID.ToString();
			//	dataRow["NoIndex"] = (i + 1).ToString();
			//	dataRow["DocNo"] = csDocList[i].DocNo;
			//	dataRow["DocTitle"] = csDocList[i].DocTitle;
			//	dataRow["Revision"] = csDocList[i].Revision;
			//	dataRow["Confidentiality"] = csDocList[i].ConfidentialityName;
			//	dataRow["DocType"] = csDocList[i].DocTypeCode;
			//	dataRow["OriginatingOrganisation"] = csDocList[i].OriginatingOrganisationName;
			//	dataRow["ReceivingOrganisation"] = csDocList[i].ReceivingOrganisationName;
			//	dataRow["GroupCode"] = csDocList[i].GroupCode;
			//	dataRow["CarbonCopy"] = csDocList[i].CarbonCopyName;
			//	dataRow["Year"] = csDocList[i].Year;
			//	dataRow["RelatedCS"] = csDocList[i].RelatedCSLNo;
			//	dataRow["NeedReply"] = csDocList[i].IsNeedReply.GetValueOrDefault() ? "X" : string.Empty;
			//	dataRow["Description"] = csDocList[i].Description;
			//	dataRow["Treatment"] = csDocList[i].Treatment;
			//	dataRow["ProposedBy"] = csDocList[i].ProposedBy;
			//	dataRow["ProposedDate"] = csDocList[i].ProposedDate != null ? csDocList[i].ProposedDate.GetValueOrDefault().ToString("dd-MMM-yyyy") : String.Empty;
			//	dataRow["ReviewedBy"] = csDocList[i].ReviewedBy;
			//	dataRow["ReviewedDate"] = csDocList[i].ReviewedDate != null ? csDocList[i].ReviewedDate.GetValueOrDefault().ToString("dd-MMM-yyyy") : String.Empty;
			//	dataRow["ApprovedBy"] = csDocList[i].ApprovedBy;
			//	dataRow["ApprovedDate"] = csDocList[i].ApprovedDate != null ? csDocList[i].ApprovedDate.GetValueOrDefault().ToString("dd-MMM-yyyy") : String.Empty;
			//	dataRow["IssuedDateFROM"] = csDocList[i].IssuedDateFrom != null ? csDocList[i].IssuedDateFrom.GetValueOrDefault().ToString("dd-MMM-yyyy") : String.Empty;
			//	dataRow["ReceivedDateTO"] = csDocList[i].ReceivedDateTo != null ? csDocList[i].ReceivedDateTo.GetValueOrDefault().ToString("dd-MMM-yyyy") : String.Empty;
			//	dataRow["ReceivedDateCC"] = csDocList[i].ReceivedDateCC != null ? csDocList[i].ReceivedDateCC.GetValueOrDefault().ToString("dd-MMM-yyyy") : String.Empty;

			//	dtCSDoc.Rows.Add(dataRow);
			//}
			//csSheet.Cells.ImportDataTable(dtCSDoc, false, 6, 1, dtCSDoc.Rows.Count, dtCSDoc.Columns.Count, true);

			//var dtMOMDoc = new DataTable();
			//dtMOMDoc.Columns.AddRange(new[]
			//{
			//	new DataColumn("DocId", typeof (String)),
			//	new DataColumn("NoIndex", typeof (String)),
			//	new DataColumn("DocNo", typeof (String)),
			...
			//	new DataColumn("OriginalDocNo", typeof (String)),
			//	new DataColumn("IssueDate", typeof (String)),
			//	new DataColumn("ResponseRequiredDate", typeof (String)),
			//	new DataColumn("ResponseTo", typeof (String)),
			//});

			//for (int i = 0; i < momDocList.Count; i++)
			//{
			//	var dataRow = dtMOMDoc.NewRow();
			//	dataRow["DocId"] = momDocList[i].ID.ToString();
			//	dataRow["NoIndex"] = (i + 1).ToString();
			//	dataRow["DocNo"] = momDocList[i].DocNo;
			//	dataRow["DocTitle"] = momDocList[i].DocTitle;
			//	dataRow["Revision"] = momDocList[i].Revision;
			//	dataRow["Confidentiality"] = momDocList[i].ConfidentialityName;
			//	dataRow["DocType"] = momDocList[i].DocTypeCode;
			//	dataRow["OriginatingOrganisation"] = momDocList[i].OriginatingOrganisationName;
			//	dataRow["ReceivingOrganisation"] = momDocList[i].ReceivingOrganisationName;
			//	dataRow["GroupCode"] = momDocList[i].GroupCode;
			//	dataRow["OriginalDocNo"] = momDocList[i].OriginalDocumentNumber;
			//	dataRow["Year"] = momDocList[i].Year;
			//	dataRow["IssueDate"] = momDocList[i].Date;
			//	dataRow["ResponseRequiredDate"] = momDocList[i].ResponseRequiredDate != null ? momDocList[i].ResponseRequiredDate.GetValueOrDefault().ToString("dd-MMM-yyyy") : String.Empty;
			//	dataRow["ResponseTo"] = momDocList[i].ResponseToName;

			//	dtMOMDoc.Rows.Add(dataRow);
			//}
			//momSheet.Cells.ImportDataTable(dtMOMDoc, false, 6, 1, dtMOMDoc.Rows.Count, dtMOMDoc.Columns.Count, true);
   //         #endregion
            workbook.Save(filePath + filename);
		...
		private void InitGridColumnView(int categoryId)
		{
			switch (categoryId)
			{
				case 3:
					...
					this.grdDocument.MasterTableView.GetColumn("ManHour").Display = false;
				...
				case 1:
					...
					this.grdDocument.MasterTableView.GetColumn("ManHour").Display = true;
