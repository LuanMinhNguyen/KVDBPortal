...
namespace EDMs.Web
{
    ...
    /// Class customer
    /// </summary>
    public partial class EAM_WORETNLineList : Page
    {
        /// <summary>
        ...
        protected void grdWORETNLine_OnNeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            var userParam = new SqlParameter("@user", SqlDbType.NVarChar);
            userParam.Value = UserSession.Current.User.Username;
            DataSet ds;
            var woPartPlanList = new List<WORETNLineDto>();
            ds = this.eamService.GetDataSet("get_WORETN_lines", new[] { userParam });
            if (ds != null)
            {
                woPartPlanList = this.eamService.CreateListFromTable<WORETNLineDto>(ds.Tables[0]);
            }

            grdWORETNLine.DataSource = woPartPlanList;
        }
    }
}
