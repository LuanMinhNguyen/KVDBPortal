<%@ Page Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="ShipmentList.aspx.cs" Inherits="EDMs.Web.Controls.CostContract.ShipmentList" EnableViewState="true" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder2" runat="server">
			...
			<telerik:RadToolBar ID="CustomerMenu" runat="server" Width="100%" OnClientButtonClicking="OnClientButtonClicking">
				<Items>
					<telerik:RadToolBarDropDown runat="server" Text="Add" ImageUrl="~/Images/addNew.png">
						<Buttons>
							<telerik:RadToolBarButton runat="server" Text="Shipment" Value="1" ImageUrl="~/Images/material.png"></telerik:RadToolBarButton>
						...
						<telerik:RadPane ID="Radpane6" runat="server" Scrolling="None">
							<telerik:RadGrid AllowCustomPaging="False" AllowPaging="True" AllowSorting="True" 
								...
								<ItemTemplate>
									...
									<a href='javascript:DeleteShipment("<%# DataBinder.Eval(Container.DataItem, "ID") %>")' style="text-decoration: none; color:blue">
									<asp:Image ID="delete" runat="server" ImageUrl="~/Images/delete.png" Style="cursor: pointer;" ToolTip="Delete Shipment" />
										<a/></div>
								</ItemTemplate>
							</telerik:GridTemplateColumn>
										 <telerik:GridTemplateColumn AllowFiltering="False">
												...
												<ItemTemplate>
													...
													<asp:Image runat="server" ImageUrl="~/Images/workflow1.png" Style="cursor: pointer;" ToolTip=" Detail"/>
														</a>
												</ItemTemplate>
											</telerik:GridTemplateColumn>
										...
										<ItemTemplate>
											...
											<asp:Image ID="AttachDoc" runat="server" ImageUrl="~/Images/attach.png" Style="cursor: pointer;" ToolTip="Attachment" />
												<a/>
										</ItemTemplate>
									</telerik:GridTemplateColumn>
							...
							<telerik:GridTemplateColumn AllowFiltering="False" UniqueName="AttachWorkflow" >
								...
								<ItemTemplate>
									...
									<asp:Image ID="imgAttachWorkflow" runat="server" ImageUrl="~/Images/attachwf.png" Style="cursor: pointer;" ToolTip="Attach To Workflow" Visible='<%# !Convert.ToBoolean(Eval("IsAttachWorkflow"))%>'/>
										<a/>
												
										<asp:Image ID="imgAttachWorkflowOk" runat="server" ImageUrl="~/Images/attachwfOk.png" ToolTip="Attached To Workflow" Visible='<%# Convert.ToBoolean(Eval("IsAttachWorkflow"))%>'/>
								</ItemTemplate>
							</telerik:GridTemplateColumn>
							...
							<telerik:GridTemplateColumn AllowFiltering="False" UniqueName="SendTrans">
										...
										<ItemStyle HorizontalAlign="Center"/>
										<ItemTemplate>
											<a href='javascript:SendShipment("<%# DataBinder.Eval(Container.DataItem, "ID") %>")' style="text-decoration: none; color:blue">
											<asp:Image ID="imgSendTrans" runat="server" ImageUrl="~/Images/sendTrans.png" Style="cursor: pointer;" ToolTip="Waiting to send Shipment" Visible='<%# !Convert.ToBoolean(Eval("IsSend"))%>'/>
											<a/>
												
											<asp:Image ID="Image1" runat="server" ImageUrl="~/Images/sendTransOk1.png" ToolTip="Shipment sent" Visible='<%# Convert.ToBoolean(Eval("IsSend"))%>'/>
										</ItemTemplate>
									</telerik:GridTemplateColumn>
									<telerik:GridTemplateColumn HeaderText="Shipment No" UniqueName="Number"
								...
								<telerik:GridTemplateColumn HeaderText="Shipment Type" UniqueName="ShipmentTypeName"
										AllowFiltering="false">
										<HeaderStyle HorizontalAlign="Center" Width="180" />
										<ItemStyle HorizontalAlign="Left" />
										<ItemTemplate>
											<%# Eval("ShipmentTypeName") %>
										</ItemTemplate>
									</telerik:GridTemplateColumn>
									<telerik:GridTemplateColumn HeaderText="Shipment Status" UniqueName="ShipmentStatusName"
										AllowFiltering="false">
										<HeaderStyle HorizontalAlign="Center" Width="180" />
										<ItemStyle HorizontalAlign="Left" />
										<ItemTemplate>
											<%# Eval("ShipmentStatusName") %>
										</ItemTemplate>
									</telerik:GridTemplateColumn>
								</Columns>
							</MasterTableView>
	...
	<telerik:RadWindowManager ID="RadWindowManager1" runat="server" EnableShadow="true" >
		<Windows>
			...
			<telerik:RadWindow ID="RevisionDialog" runat="server" Title="Shipment Detail"
			...
			<telerik:RadWindow ID="ImportEMDRReport" runat="server" Title="Import EMDR Report"
			 ...
			 <telerik:RadWindow ID="AttachWorkflow" runat="server" Title="Attach to Workflow"
	...
	<telerik:RadCodeBlock ID="RadCodeBlock1" runat="server">
		...
		<script type="text/javascript">
			...
			function DeleteWorkflow(Id) {
			 ...
			 function AttachWorkflow(objId) {
				var owd = $find("<%=AttachWorkflow.ClientID %>");
				 ...
				 owd.Show();
				 owd.setUrl("../../Controls/Workflow/AttachWorkflow.aspx?objId=" + objId + "&projId=" + projId + "&type=6", "AttachWorkflow");
			}
			...
			function SendShipment(objId) {
				if (confirm("Do you want to send this shipment?") == false) return;
				ajaxManager.ajaxRequest("SendShipment_" + objId);
			}
 ...
 function ShowWorkflowProcessHistory(docId) {
			...
			function ShowEditForm(docId, projId) {
				var owd = $find("<%=CustomerDialog.ClientID %>");
				owd.Show();
				owd.setUrl("ShipmentEditForm.aspx?projId=" + projId + "&ShipmentId=" + docId, "CustomerDialog");
			}
			...
			function ShowAttachFile(Id) {	
					var owd = $find("<%=AttachDoc.ClientID %>");
					owd.Show();
					owd.setUrl("ShipmentAttachFile.aspx?ShipmentId=" + Id, "AttachDoc");
			}
			...
			function DeleteShipment(objId) {
				if (confirm("Do you want to delete item?") == false) return;
				ajaxManager.ajaxRequest("DeleteShipment_" + objId);
			}
			 function ShowStep(objId) {
			  var owd = $find("<%=RevisionDialog.ClientID %>");
				owd.Show();
				owd.maximize(true);
				owd.setUrl("ShipmentDetailList.aspx?shipmentId=" + objId, "RevisionDialog");
			}
			function OnClientButtonClicking(sender, args) {
				...
				if (strText.toLowerCase() == "shipment") {
					...
					var owd = $find("<%=CustomerDialog.ClientID %>");
					owd.Show();
					owd.setUrl("ShipmentEditForm.aspx?projId=" + projectId, "CustomerDialog");
					//}
				}
