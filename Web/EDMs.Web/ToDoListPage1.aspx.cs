...
namespace EDMs.Web
{
    ...
    /// Class customer
    /// </summary>
    public partial class ToDoListPage1 : Page
    {
        ...
        {
            get { return this.objAssignedUserService.GetAllIncompleteByUser(UserSession.Current.User.Id).Where(t=> t.IsLeaf.GetValueOrDefault()).ToList(); }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                {
                    GridColumn rowStateColumn = grdDocument.MasterTableView.GetColumnSafe("ObjectNumber");
        ...
        protected void grdInfor_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            var list = new List<ObjectAssignedUser>();
            list = this.objAssignedUserService.GetAllIncompleteByDoc().Where(t=> t.ActionTypeId==2).ToList();
            var ListNextStep= new List<ObjectAssignedUser>();
            var CurentstepReview = new List<ObjectAssignedUser>();
            foreach (var currentWorkAssignedUser in list)
            {
                   ...
                   // var pendingWorkAssignUserList = this.objAssignedUserService.GetAllIncompleteByDoc(currentWorkAssignedUser.ObjectID.GetValueOrDefault(), wfObj.ID);
                    if (wfObj!= null)
                    {
                        ...
                        if (currentWorkAssignedWf != null)
                        {
                            ...
                            if (nextStep != null)
                            {
                                if (ListNextStep.Where(t => t.ObjectID == docObj.ID).ToList().Count == 0)
                                {
                                    //this.ProcessWorkflowTem(nextStep, wfObj, docObj, ref ListNextStep, currentWorkAssignedUser);
                                }
                            }
           ...
           foreach(var step in ListNextStep)
            {
                step.WorkingStatus = GetNameUser(list.Where(t => t.ObjectNumber == step.ObjectNumber).ToList());
            }

            CurentstepReview = ListNextStep.Where(t => t.ActionTypeId == 3 && t.UserID == UserSession.Current.User.Id).ToList();
            this.grdInfor.DataSource = CurentstepReview.OrderByDescending(t=> t.ReceivedDate).ToList();
        }
        ...
        protected void grdInfor_ItemDataBound(object sender, GridItemEventArgs e)
        {
            if (e.Item is GridDataItem)
            {
                var item = e.Item as GridDataItem;
                if (item["IsOverDue"].Text == "True")
                {
