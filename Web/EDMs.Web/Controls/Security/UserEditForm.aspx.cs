...
namespace EDMs.Web.Controls.Security
{
    public partial class UserEditForm : Page
    {
        ...
        private void LoadDataToCombo()
        {
            ...
            var ytpList = this.userService.GetAll().Where(t => t.IsLeader.GetValueOrDefault()).ToList();
            //ytpList.Insert(0, new User(){Id=0, FullName = string.Empty});
            this.ddlYTP.DataSource = ytpList;
            this.ddlYTP.DataTextField = "FullName";
            this.ddlYTP.DataValueField = "Id";
            this.ddlYTP.DataBind();

            this.ddlMYTP.DataSource = ytpList;
            this.ddlMYTP.DataTextField = "FullName";
            this.ddlMYTP.DataValueField = "Id";
            this.ddlMYTP.DataBind();
        }
            ...
            if (!IsPostBack)
            {
                ...
                if (!string.IsNullOrEmpty(this.Request.QueryString["userId"]))
                {
                    ...
                    if (user != null)
                    {
                        ...
                        this.cbYTP.Checked = user.IsLeader.GetValueOrDefault();
                        this.cbYTHC.Checked = !user.IsLeader.GetValueOrDefault();
                        this.ddlYTP.SelectedValue = user.ManagerId.GetValueOrDefault().ToString();
                        ...
                        foreach (RadComboBoxItem item in this.ddlMYTP.Items)
                        {
                            item.Checked = !string.IsNullOrEmpty(user.ManagerIds) && user.ManagerIds.Split('$').Contains(item.Value);
                        }
                ...
                if (departmentObj != null)
                {
                    if (!string.IsNullOrEmpty(this.Request.QueryString["userId"]))
                    {
                        ...
                        if (user != null)
                        {
                            ...
                            user.IsLeader = this.cbYTP.Checked;
                            //user.ManagerId = Convert.ToInt32(this.ddlYTP.SelectedValue);
                            //user.ManagerName = this.ddlYTP.SelectedItem.Text;

                            user.ManagerIds = string.Empty;
                            user.ManagerName = string.Empty;

                            foreach (RadComboBoxItem item in this.ddlMYTP.CheckedItems)
                            {
                                user.ManagerIds += item.Value + "$";
                                user.ManagerName += item.Text + "</br>";
                            }
