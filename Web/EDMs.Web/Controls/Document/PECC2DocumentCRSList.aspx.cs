...
namespace EDMs.Web.Controls.Document
{
    ...
    /// The customer edit form.
    /// </summary>
    public partial class PECC2DocumentCRSList : Page
    {
        ...
        private void ReuploadFileCRS()
        {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["objId"]))
            {
                ...
                var crsList =
             this.pecc2TransmittalAttachFileService.GetByTrans(objId)
                 .FirstOrDefault(t => t.TypeId == 2);
                ...
                if (crsList != null)
                {
                    try
                    {
                        if (File.Exists(Server.MapPath(@"../.." + crsList.FilePath)))
                        {
                            File.Delete(Server.MapPath(@"../.." + crsList.FilePath));
                        }
                        this.pecc2TransmittalAttachFileService.Delete(crsList);
                    }
                    catch { }
                }
        ...
        protected void grdAttachCRSFile_OnNeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            ...
            {
                this.grdAttachCRSFile.DataSource = new List<PECC2TransmittalAttachFileService>();
            }
        }
