<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ReaderControl.aspx.cs" Inherits="WebAppPdf.WebViewer.lib.html5.ReaderControl" %>
...
<head>
    ...
    <link rel="stylesheet" href="customs/bootstrap.min.css">
...
<body>
    <script>
        function Savelayer() {
            ...
            var strxmldataheade = "";
            var strxmldatacontent = "";
            var colorselect = $('#slectedcolor').val();
            if (colorselect == "")
                colorselect = "#000";
            var list = document.getElementsByClassName("noteSubject");
            var listcontent = document.getElementsByClassName("staticNoteContents");
            if (list.length > 0) {
                for (var i = 0, length = list.length; i < length; i++) {
                    // var newLI = document.createElement("li");
                    // ul.appendChild(newLI);
                    //newLI.innerHTML = list[i].innerHTML;
                    strxmldataheade += "|" + list[i].innerText;
                    strxmldatacontent += "|" + listcontent[i].innerText;
                    //alert(list[i].innerHTML);
                    //nodeList[i].innerHTML = content;
                }
                var dataValue = "{ xml: '" + strxmldataheade + "', xml1: '" + strxmldatacontent + "', color: '" + colorselect + "'}";
                $.ajax({
                    type: "POST",
                    url: "savelayer.aspx/Savelayer",
                    data: dataValue,
                ...
                alert("Không có chỉnh sửa, comment trên tài liệu này");
            }
        ...
        function ConfirmConsolidate()
        {
            $.ajax({
                type: "POST",
                url: "savelayer.aspx/Consolidate",
                data: '',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: OnSuccess,
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Consolidate Layer</h4>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <div class="col-lg-12 ">
                            <h4>Số người đã thao tác với tài liệu</h4>
                            <hr />
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    <% WebAppPdf.EditPDFControlPara.lstConsolidate = new List<WebAppPdf.LayerPDF>();
                                        int indexLayer = 0;
                                        foreach (var item in WebAppPdf.EditPDFControlPara.CurrentMarkupFileList)
                                        {
                                            ...
                                            if (System.IO.File.Exists(fileXml))
                                            {
                                                try
                                                {
                                                    ...
                                                    WebAppPdf.EditPDFControlPara.lstConsolidate.Add(new WebAppPdf.LayerPDF { color=color, item1=item1, item2=item2, Content=Content, Datestr=Datestr, User=User});
                                    %>
                                    <tr>
                                        <th scope="row">
                                            <input type="checkbox" class="form-control" /></th>
                                        <td style="padding:5px;"><span style="width:10px;height:10px;background-color:<%=color %>">Color</span></td>
                                        <td><%=item1 %><%=Content %><%=item2 %></td>
                                        <td><%=User %></td> 
                                        <td><%=Datestr%></td>
                                        <td><a target="_blank" href="../../../Default.aspx?layer=<%=indexLayer%>">Open</a></td>
                                    </tr>
                                    <% indexLayer++; }catch(Exception ex)
                                                {
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal" onclick="return ConfirmConsolidate();">Consolidate</button>
                ...
                <div id="tabs-3">
                    <div id="searchView" class="tab-panel-stretch">
                        ...
                        <div class="tab-panel-item-fixed">
                            <input id="wholeWordSearch" type="checkbox"><span class="ui-label" data-i18n="sidepanel.searchTab.wholeWordOnly"></span>
                        </div>
                        <div class="tab-panel-item-fixed">
        ...
        <div class="right-content">
            ...
            <div id="control" class="ui-widget-header toolbar">
                ...
                <div class="right-aligned">
                    <div class="group">
                        <span id="btnConsolidate" data-toggle="modal" data-target="#myModal" class="glyphicons package" title="Consolidate"></span>
                        <span onclick="Savelayer();" class="glyphicons floppy_save" runat="server" title="Save layer"></span>
                        ...
                        <div id="toolList" class="toggleControl" style="margin-left: 6px">
                            ...
                            <span id="overflowTools" class="moreButton glyphicons down_arrow" data-i18n="[title]annotations.tooltips.more">
                                <div id="overflowToolsContainer" class="toolsContainer">
                                    <span data-toolmode="" class="annotTool glyphicons  picture" onclick="addimage();"></span>
                                    ...
                                    <span data-toolmode="AnnotationEdit" class="annotTool glyphicons customicons multiselect" data-i18n="[title]annotations.tooltips.edit"></span>
                                </div>
                            </span>
                        </div>
                    </div>

                    <div class="group hidden-sm">
            ...
            <ul id="displayModeMenuList" class="ui-widget ui-menu-dropdown" style="display: none">
                <li data-display-mode="single"><a href="javascript:void(0)"><span class="ui-icon ui-icon-custom-page-single"></span>
                    <div data-i18n="controlbar.layoutMode.single"></div>
                </a></li>
                <li data-display-mode="single-cont"><a href="javascript:void(0)"><span class="ui-icon ui-icon-custom-page-single-cont"></span>
                    <div data-i18n="controlbar.layoutMode.continuous"></div>
                </a></li>
                <li data-display-mode="facing"><a href="javascript:void(0)"><span class="ui-icon ui-icon-custom-page-facing"></span>
                    <div data-i18n="controlbar.layoutMode.facing"></div>
                </a></li>
                <li data-display-mode="facing-cont"><a href="javascript:void(0)"><span class="ui-icon ui-icon-custom-page-facing-cont"></span>
                    <div data-i18n="controlbar.layoutMode.facingContinuous"></div>
                </a></li>
                <li data-display-mode="cover"><a href="javascript:void(0)"><span class="ui-icon ui-icon-custom-page-cover"></span>
                    <div data-i18n="controlbar.layoutMode.cover"></div>
                </a></li>
                <li data-display-mode="cover-cont"><a href="javascript:void(0)"><span class="ui-icon ui-icon-custom-page-cover-cont"></span>
                    <div data-i18n="controlbar.layoutMode.coverContinuous"></div>
                </a></li>
            </ul>
            ...
            <div id="annotEditDialog" style="display: none">
                ...
                <div id="annotEditProperties" style="display: none">
                    <div id="colorType">
                        ...
                        <input type="hidden" id="slectedcolor" name="slectedcolor" />
                    </div>

                    <div id="basicProperties">
                        <div id="colorPicker">
                            <ul class="colorPicker">
                                <li id="addColorButton">
                                    <div></div>
                                </li>
                                <li id="removeColorButton">
                                    <div></div>
                                </li>
                            </ul>
                        </div>
    ...
    <script src="external/jquery-2.1.0.min.js"></script>
