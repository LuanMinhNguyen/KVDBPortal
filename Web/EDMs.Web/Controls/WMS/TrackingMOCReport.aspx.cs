...
namespace EDMs.Web.Controls.WMS
{
    ...
    /// The customer edit form.
    /// </summary>
    public partial class TrackingMOCReport : Page
    {
        ...
        protected void grdDocument_OnNeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            if (Request.QueryString["stausName"] != null && Request.QueryString["projId"] != null)
            {
                ...
                var mrListFilter = new List<TrackingMOC>();

                var mrList = this.mocService.GetAll(projId, string.Empty).Where(t => (fromDate == null || t.CreatedDate >= fromDate)
                                                                                && (toDate == null || t.CreatedDate < toDate.Value.AddDays(1))).ToList();
                var mroverDueTask = this.objAssignedUserService.GetAllOverDueTask("MOC").Select(t => t.ObjectID).ToList();
                switch (statusName)
                {
                    case "Pending":
                ...
                case "DocDetail":
                    {
                        ...
                        e.DetailTableView.DataSource = this.mocAttachFileService.GetByMOC(objId);
                        break;
                    }
            }
        }
