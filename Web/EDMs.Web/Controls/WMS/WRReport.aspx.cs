...
namespace EDMs.Web.Controls.WMS
{
    ...
    /// The customer edit form.
    /// </summary>
    public partial class WRReport : Page
    {
        ...
        protected void grdDocument_OnNeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            if (Request.QueryString["stausName"] != null && Request.QueryString["projId"] != null)
            {
                ...
                var mrListFilter = new List<WorkRequest>();

                var mrList = this.wrService.GetAll(projId, string.Empty).Where(t => (fromDate == null || t.RaisedDate >= fromDate)
                                                                                && (toDate == null || t.RaisedDate < toDate.Value.AddDays(1))).ToList();
                var mroverDueTask = this.objAssignedUserService.GetAllOverDueTask("Work Request").Select(t => t.ObjectID).ToList();
                switch (statusName)
                {
                    case "Pending":
                ...
                case "DocDetail":
                    {
                        ...
                        e.DetailTableView.DataSource = this.wrAttachFileService.GetByWR(objId);
                        break;
                    }
            }
        }
