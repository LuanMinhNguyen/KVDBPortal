...
namespace EDMs.Web.Controls.WMS
{
    ...
    /// The customer edit form.
    /// </summary>
    public partial class TrackingPunchReport : Page
    {
        protected const string ServiceName = "EDMSFolderWatcher";

        private readonly TrackingPunchService pmService;

        private readonly TrackingPunchAttachFileService pmAttachFileService;


        /// <summary>
        /// Initializes a new instance of the <see cref="TrackingPunchReport"/> class.
        /// </summary>
        public TrackingPunchReport()
        {
            this.pmAttachFileService = new TrackingPunchAttachFileService();
            this.pmService = new TrackingPunchService();
        ...
        protected void grdDocument_OnNeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            if(Request.QueryString["stausName"] != null && Request.QueryString["projId"] != null)
            {
                ...
                var pmListFilter = new List<TrackingPunch>();
                var pmList = this.pmService.GetAllByProject(projId);

                switch (statusName)
                {
                    case "AC":
                        pmListFilter = pmList.Where(t => t.Status == "AC").ToList();
                        break;
                    case "BC":
                        pmListFilter = pmList.Where(t => t.Status == "BC").ToList();
                        break;
                    case "Cancel":
