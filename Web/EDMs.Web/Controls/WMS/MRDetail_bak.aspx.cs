    ...
    public partial class MRDetail : Page
    {
        ...
        private void LoadInitData()
        {
            ...
            if (mrObj != null)
            {
                ...
                this.txtOriginatorDate.SelectedDate = mrObj.OriginatorDate;

                this.txtStoremanName.Text = mrObj.StoreManName;
                this.txtStoremanDate.SelectedDate = mrObj.StoreManDate;

                this.txtSupervisorName.Text = mrObj.SupervisorName;
                this.txtSupervisorDate.SelectedDate = mrObj.SupervisorDate;

                this.txtOIMName.Text = mrObj.OIMName;
                this.txtOIMDate.SelectedDate = mrObj.OIMDate;
            }
        ...
        protected void btnExportMR_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(this.Request.QueryString["mrId"]))
            {
                ...
                if (mrObj != null)
                {
                    ...
                    dataSheet.Cells["E" + (10 + mrDetailList.Count)].PutValue(mrObj.OriginatorDate?.ToString("dd/MM/yyyy") ?? string.Empty);

                    dataSheet.Cells["I" + (9 + mrDetailList.Count)].PutValue(mrObj.StoreManName);
                    dataSheet.Cells["J" + (10 + mrDetailList.Count)].PutValue(mrObj.StoreManDate?.ToString("dd/MM/yyyy") ?? string.Empty);

                    dataSheet.Cells["K" + (9 + mrDetailList.Count)].PutValue(mrObj.SupervisorName);
                    dataSheet.Cells["L" + (10 + mrDetailList.Count)].PutValue(mrObj.SupervisorDate?.ToString("dd/MM/yyyy") ?? string.Empty);

                    dataSheet.Cells["N" + (9 + mrDetailList.Count)].PutValue(mrObj.OIMName);
                    dataSheet.Cells["O" + (10 + mrDetailList.Count)].PutValue(mrObj.OIMDate?.ToString("dd/MM/yyyy") ?? string.Empty);
        ...
        protected void btnSaveMRSignInfo_Click(object sender, EventArgs e)
        {
            ...
            if (mrObj != null)
            {
                ...
                mrObj.OriginatorDate = this.txtOriginatorDate.SelectedDate ;

                mrObj.StoreManName = this.txtStoremanName.Text ;
                mrObj.StoreManDate = this.txtStoremanDate.SelectedDate ;

                mrObj.SupervisorName = this.txtSupervisorName.Text ;
                mrObj.SupervisorDate = this.txtSupervisorDate.SelectedDate ;

                mrObj.OIMName = this.txtOIMName.Text ;
                mrObj.OIMDate = this.txtOIMDate.SelectedDate ;
                this.mrService.Update(mrObj);
            }
        }
        ...
        protected void grdCheckList_OnBatchEditCommand(object sender, GridBatchEditingEventArgs e)
        {
            foreach (GridBatchEditingCommand command in e.Commands)
            {
                ...
                var remark = newValues["Remark"].ToString();
        ...
        {
            var mrId = new Guid(this.Request.QueryString["mrId"]);
            this.grdMRDetailTracking.DataSource = this.mrDetailService.GetByMR(mrId).OrderBy(t => t.SFICode);
        }
