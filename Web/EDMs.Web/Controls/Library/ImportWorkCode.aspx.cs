...
namespace EDMs.Web.Controls.Library
{
    ...
    /// The customer edit form.
    /// </summary>
    public partial class ImportWorkCode : Page
    {
                ...
                foreach (UploadedFile docFile in this.docuploader.UploadedFiles)
                {
                    ...
                    if (extension == ".xls" || extension == ".xlsx" || extension == ".xlsm")
                    {
                        ...
                        foreach (DataRow prDetailRow in prDetailTable.Rows)
                        {
                            ...
                            var workCodeObj = this.workService.GetById(int.Parse(ID));
                            if (workCodeObj != null)
                            {
                                workCodeObj.Code = Code;
                                workCodeObj.Description = Description;
                                workCodeObj.LastUpdatedBy = UserSession.Current.User.Id;
                                workCodeObj.LastUpdatedDate = DateTime.Now;
                                workService.Update(workCodeObj);
                            }
                            else
                            {
                                workCodeObj = new WorkCode();
                                workCodeObj.Code = Code;
                                workCodeObj.Description = Description;
                                workCodeObj.CreatedBy = UserSession.Current.User.Id;
                                workCodeObj.CreatedDate = DateTime.Now;
                                this.workService.Insert(workCodeObj);
                            }
                        }
