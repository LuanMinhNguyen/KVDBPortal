...
namespace EDMs.Web.Controls.Library
{
    ...
    /// Class customer
    /// </summary>
    public partial class KKSList : Page
    {
        ...
        private void ExportKKS()
        {
            var filePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"Exports") + @"\";
            //var filePath = Server.MapPath("Exports") + @"\";
            var workbook = new Workbook();
            workbook.Open(filePath + @"Template\TemplateKKS.xls");

            var dataSheet = workbook.Worksheets[0];

            var dtFull = new DataTable();

            var KKSIdentificationCodeList = this.KKSIdentificationCodeService.GetAll().ToList();
            ...
            foreach (var kksObj in KKSIdentificationCodeList)
            {
                var dataRow = dtFull.NewRow();
                dataRow["ID"] = kksObj.ID;
                dataRow["Code"] = kksObj.Code;
                dataRow["Description"] = kksObj.Description;
            ...
            dataSheet.Cells["C1"].PutValue("KKS Identification Code");
            var filename = "KKS Identification Code List.xls";
        ...
        {
            var projectList = this.KKSIdentificationCodeService.GetAll().OrderBy(t => t.Code);
        ...
        protected void grdDocument_DeleteCommand(object sender, GridCommandEventArgs e)
        {
            ...
            this.KKSIdentificationCodeService.Delete(disId);
        ...
        private void LoadScopePanel()
        {
            ...
            if (permissions.Any())
            {
                ...
                foreach (RadPanelItem item in this.radPbScope.Items[0].Items)
                {
                    item.ImageUrl = permissions.FirstOrDefault(t => t.Id == Convert.ToInt32(item.Value)).Menu.Icon;
                    item.NavigateUrl = permissions.FirstOrDefault(t => t.Id == Convert.ToInt32(item.Value)).Menu.Url;
                    if (item.Text == "KKS Identification Code")
                    {
                        item.Selected = true;
                    }
                }
