...
namespace EDMs.Web.Controls.Library
{
    ...
    /// Class customer
    /// </summary>
    public partial class ChangeRequestTypeList : Page
    {
        private readonly PermissionService permissionService = new PermissionService();

        private readonly ChangeRequestTypeService ChangeRequestTypeCodeService = new ChangeRequestTypeService();
        ...
        protected void RadAjaxManager1_AjaxRequest(object sender, AjaxRequestEventArgs e)
        {
            ...
            {
                this.ExportChangeRequestType();
            }
        }

        private void ExportChangeRequestType()
        {
            ...
            var ChangeRequestTypeCodeList = this.ChangeRequestTypeCodeService.GetAll().ToList();
            ...
            foreach (var ChangeRequestTypeObj in ChangeRequestTypeCodeList)
            {
                var dataRow = dtFull.NewRow();
                dataRow["ID"] = ChangeRequestTypeObj.ID;
                dataRow["Code"] = ChangeRequestTypeObj.Code;
                dataRow["Description"] = ChangeRequestTypeObj.Description;
            ...
            dataSheet.Cells["C1"].PutValue("ChangeRequestTypeCode CODE");
            var filename = "ChangeRequestType.xlsx";
        ...
        {
            var projectList = this.ChangeRequestTypeCodeService.GetAll();
        ...
        protected void grdDocument_DeleteCommand(object sender, GridCommandEventArgs e)
        {
            ...
            this.ChangeRequestTypeCodeService.Delete(disId);
        ...
        private void LoadScopePanel()
        {
            ...
            if (permissions.Any())
            {
                ...
                foreach (RadPanelItem item in this.radPbScope.Items[0].Items)
                {
                    item.ImageUrl = permissions.FirstOrDefault(t => t.Id == Convert.ToInt32(item.Value)).Menu.Icon;
                    item.NavigateUrl = permissions.FirstOrDefault(t => t.Id == Convert.ToInt32(item.Value)).Menu.Url;
                    if (item.Text == "Change Request Type")
                    {
                        item.Selected = true;
                    }
                }
