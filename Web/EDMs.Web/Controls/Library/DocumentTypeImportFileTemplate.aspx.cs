...
namespace EDMs.Web.Controls.Library
{
    ...
    /// </summary>
    public partial class DocumentTypeImportFileTemplate : Page
    {

        private readonly DocumentTypeService _DocumentTypeSevice= new DocumentTypeService();
            ...
            try
            {
                ...
                foreach (UploadedFile docFile in this.docuploader.UploadedFiles)
                {
                    ...
                    if (extension == ".xls" || extension == ".xlsx" || extension == ".xlsm")
                    {
                        ...
                        {
                            var Parent = dataRow["Column1"].ToString();
                            var DocTypeCode = dataRow["column2"].ToString();
                            var description = dataRow["column3"].ToString();

                            var Obj = this._DocumentTypeSevice.GetByCode(Parent);
                            if (! String.IsNullOrEmpty(DocTypeCode))
                            {
                                var areaobj = new DocumentType()
                                {
                                    Code = DocTypeCode,
                                ...
                                this._DocumentTypeSevice.Insert(areaobj);
                            }
                            else
                            {
                                error = "Have Error";
                ...
                if (error.Length != 0)
                {
                    ...
                    this.lblError.Text = "Have error at template Form: Code is empty.</br>";
