...
namespace EDMs.Web
{
    ...
    /// Class customer
    /// </summary>
    public partial class EAM_ReceiptList : Page
    {
        /// <summary>
        ...
        protected void grdReceiptline_OnNeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            ...
            DataSet ds;
            var woPartPlanList = new List<ReceiptLineDto>();
            ds = this.eamService.GetDataSet("get_Receipts_lines", new[] { userParam });
            if (ds != null)
            {
                woPartPlanList = this.eamService.CreateListFromTable<ReceiptLineDto>(ds.Tables[0]);
            }

            grdReceiptline.DataSource = woPartPlanList;
        }
    }
}
