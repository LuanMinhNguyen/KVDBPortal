...
"Please use an HTML5 compatible browser";$(me.element).append('<div id="webviewer-browser-unsupported">'+supportError+".</div>")}},_viewerLoaded:function(iframe){var me=this;me._trigger("ready");var viewerWindow=iframe.contentWindow;var outerWindow=window;outerWindow.CoreControls=outerWindow.CoreControls||{};outerWindow.CoreControls.DisplayModes=viewerWindow.CoreControls.DisplayModes;if(typeof me.options.encryption!=="undefined"){var doc=decodeURIComponent(me.options.initialDoc);var loadOptions={decrypt:viewerWindow.CoreControls.Encryption.decrypt,
...
function(event,data1,data2){me._trigger(event.type,null,data1,data2)})},_getHTML5OptionsURL:function(){if(this.selectedType==="html5"){if(this.options.html5Options)$.extend(this.options,this.options.html5Options)}else if(this.selectedType==="html5Mobile")if(this.options.html5MobileOptions)$.extend(this.options,this.options.html5MobileOptions);var options=this.options;var url="";var me=this;function acceptPdfUrl(){url+=options.pdfBackend?"&pdf="+options.pdfBackend:"&pdf=auto"}if(typeof options.initialDoc!==
"undefined")url+="#d="+options.initialDoc;if(options.documentType==="pdf")if(typeof options.backendType==="undefined")acceptPdfUrl();else url+="&pdf=nodejs";else if(typeof options.documentType==="undefined"&&typeof options.initialDoc!=="undefined"&&options.initialDoc.toLowerCase().slice(-4)===".pdf")acceptPdfUrl();if(options.streaming)url+="&streaming=true";if(options.externalPath){var path=this._correctRelativePath(options.externalPath);path=encodeURIComponent(path);url+="&p="+path}if(options.encryption)url+=
"&auto_load=false";if(options.enableAnnotations)url+="&a=1";if(options.serverUrl){var serverUrl=this._correctRelativePath(options.serverUrl);serverUrl=encodeURIComponent(serverUrl);url+="&server_url="+serverUrl}if(options.documentId)url+="&did="+options.documentId;if(options.config){var config=this._correctRelativePath(options.config);config=encodeURIComponent(config);url+="&config="+config}if(options.enableOfflineMode)url+="&offline=1";if(options.startOffline)url+="&startOffline=1";if(options.enableReadOnlyMode)url+=
...
url+="&xdomain_urls="+encodeURIComponent(JSON.stringify(urls))}if(options.azureWorkaround)url+="&azureWorkaround=1";if(!options.useDownloader)url+="&useDownloader=0";if(options.subzero)url+="&subzero=1";if(typeof options.workerTransportPromise!=="undefined")url+="&useSharedWorker="+(options.workerTransportPromise?"true":"false");if(url.length>0&&url[0]==="&")url="#"+url.slice(1);return url},_createHTML5:function(){var me=this;var iframeSource=this.options.html5Path+this._getHTML5OptionsURL();var $rcFrame=
$(document.createElement("iframe"));$rcFrame.attr({id:this.rcId,src:iframeSource,frameborder:0,width:"100%",height:"100%",allowFullScreen:true,webkitallowfullscreen:true,mozallowfullscreen:true});if(this.options.backgroundColor)$rcFrame.attr("data-bgcolor",this.options.backgroundColor);$rcFrame.load(function(){me.instance=this.contentWindow.readerControl;var iframe=this;if(typeof me.instance==="undefined")this.contentWindow.$(this.contentDocument).bind("viewerLoaded",function(event){me.instance=iframe.contentWindow.readerControl;
me._viewerLoaded(iframe)});else me._viewerLoaded(iframe)});$(this.element).append($rcFrame);return $rcFrame},_createHTML5Mobile:function(){var me=this;var iframeSource=this.options.html5MobilePath+this._getHTML5OptionsURL();var $rcFrame=$(document.createElement("iframe"));$rcFrame.attr({id:this.rcId,src:iframeSource,frameborder:0});$rcFrame.css("width","100%").css("height","100%");$rcFrame.load(function(){me.instance=this.contentWindow.readerControl;var iframe=this;if(typeof me.instance==="undefined")this.contentWindow.$(this.contentDocument).bind("viewerLoaded",
