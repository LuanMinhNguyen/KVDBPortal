...
namespace EDMs.Web
{
    ...
    /// Class customer
    /// </summary>
    public partial class MorningCall : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            Session.Add("SelectedMainMenu", "MR Management");
        ...
        protected void grdDocument_OnNeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            if (this.ddlProject.SelectedItem != null)
            {
                ...
                var objList = new List<TrackingMorningCall>();
                var deptId = this.rtvDiscipline.SelectedNode != null
                    ? Convert.ToInt32(this.rtvDiscipline.SelectedNode.Value)
                    : 0;

                switch (ddlStatus.SelectedValue)
                {
                    case "1":
                        //objList = this.rtvDiscipline.SelectedNode != null
                        //            ? this.MorningCallService.GetAll(projectId, txtSearchAllField.Text.Trim()).Where(t => t.DeparmentId == deptId).ToList()
                        //            : this.MorningCallService.GetAll(projectId, txtSearchAllField.Text.Trim());

                        objList = this.morningCallService.GetAll(projectId, txtSearchAllField.Text.Trim());
                        break;
                    case "4":
                        var overDueTask = this.objAssignedUserService.GetAllOverDueTask("MorningCall").Select(t => t.ObjectID).ToList();
                        //objList = this.rtvDiscipline.SelectedNode != null
                        //            ? this.mrService.GetAll(projectId, txtSearchAllField.Text.Trim()).Where(t => t.DeparmentId == deptId && overDueTask.Contains(t.ID)).ToList()
                        //            : this.mrService.GetAll(projectId, txtSearchAllField.Text.Trim()).Where(t => overDueTask.Contains(t.ID)).ToList();

                        objList = this.morningCallService.GetAll(projectId, txtSearchAllField.Text.Trim()).Where(t => overDueTask.Contains(t.ID)).ToList();
                        break;
                    case "5":
                        //objList = this.rtvDiscipline.SelectedNode != null
                        //             ? this.mrService.GetAllCompletedMR(projectId, txtSearchAllField.Text.Trim()).Where(t => t.DeparmentId == deptId).ToList()
                        //             : this.mrService.GetAllCompletedMR(projectId, txtSearchAllField.Text.Trim());

                        objList = this.morningCallService.GetAllCompletedMorningCall(projectId, txtSearchAllField.Text.Trim());
                        break;
                    case "6":
                        //objList = this.rtvDiscipline.SelectedNode != null
                        //            ? this.mrService.GetAllIncompleteMR(projectId, txtSearchAllField.Text.Trim()).Where(t => t.DeparmentId == deptId).ToList()
                        //            : this.mrService.GetAllIncompleteMR(projectId, txtSearchAllField.Text.Trim());

                        objList = this.morningCallService.GetAllIncompleteMorningCall(projectId, txtSearchAllField.Text.Trim());
                        break;

                    case "7":
                        //objList = this.rtvDiscipline.SelectedNode != null
                        //            ? this.mrService.GetAllCompletedMRWaitingPrint(projectId, txtSearchAllField.Text.Trim()).Where(t => t.DeparmentId == deptId).ToList()
                        //            : this.mrService.GetAllCompletedMRWaitingPrint(projectId, txtSearchAllField.Text.Trim());

                        objList = this.morningCallService.GetAllCompletedMorningCallWaitingPrint(projectId, txtSearchAllField.Text.Trim());
                        break;
                }
        ...
        private void LoadObjectTree()
        {
            ...
            var departmentList = this.roleService.GetAll(false).Where(t => t.TypeId == 2);
            this.rtvDiscipline.DataSource = departmentList;
            ...
            foreach (RadTreeNode node in this.rtvDiscipline.Nodes)
            {
                var deptItem = departmentList.FirstOrDefault(t => t.Id == Convert.ToInt32(node.Value));
                if (deptItem != null && !string.IsNullOrEmpty(deptItem.Color))
                {
                    node.BackColor = ColorTranslator.FromHtml(deptItem.Color);
                }
            }
        }
