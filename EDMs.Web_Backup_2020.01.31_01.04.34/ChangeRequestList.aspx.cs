...
namespace EDMs.Web
{
    ...
    /// Class customer
    /// </summary>
    public partial class ChangeRequestList : Page
    {
        ...
        private readonly ChangeRequestTypeService changeRequestTypeService = new ChangeRequestTypeService();

        private readonly ChangeGradeCodeService changeGradeCodeService = new ChangeGradeCodeService();

        private readonly ChangeRequestReviewResultCodeService changeRequestReviewResultCodeService = new ChangeRequestReviewResultCodeService();
        ...
        protected void LoadDocuments(bool isbind = false, bool isListAll = false)
        {
            ...
            var allChangeRequestOfProject = this.changeRequestService.GetAllByProject(projectId, UserSession.Current.User.ConfidentialId.GetValueOrDefault());
            ...
            if (this.rtvTreeNode.SelectedNode!= null )
            {
                ...
                if (!string.IsNullOrEmpty(tagnode))
                {
                    ...
                    {
                        case "Area":
                            changeRequestList = allChangeRequestOfProject.Where(t => selectedNodesId.Contains(t.AreaId.GetValueOrDefault())).ToList();
                            break;
                        case "Unit":
                            changeRequestList = allChangeRequestOfProject.Where(t => selectedNodesId.Contains(t.UnitId.GetValueOrDefault())).ToList();
                            break;
                        case "ChangeRequestType":
                            changeRequestList = allChangeRequestOfProject.Where(t => selectedNodesId.Contains(t.TypeId.GetValueOrDefault())).ToList();
                            break;
                        case "ChangeGradeCode":
                            changeRequestList = allChangeRequestOfProject.Where(t => selectedNodesId.Contains(t.ChangeGradeCodeId.GetValueOrDefault())).ToList();
                            break;
                        case "Group":
                            changeRequestList = allChangeRequestOfProject.Where(t => selectedNodesId.Contains(t.GroupId.GetValueOrDefault())).ToList();
                            break;
                    }
                }
            }
            ...
            {
                changeRequestList = allChangeRequestOfProject;
            }
            
            this.grdDocument.DataSource = changeRequestList.OrderBy(t =>t.Number);
        }
        ...
        private void LoadObjectTree()
        {
            ...
            // Add Change Request Type 
            var changeRequestTypeList = this.changeRequestTypeService.GetAll();
            var root = new RadTreeNode();
            root.Text = "Change Request Type";
            root.ImageUrl = "~/Images/index.png";
            root.Value = "-1";
            root.Target = "ChangeRequestType";
            root.Expanded = true;
            foreach(var type  in changeRequestTypeList)
            {
                RadTreeNode node = new RadTreeNode();
                node.Text = type.Code + ", " + type.Description;
                node.Value = type.ID.ToString();
                node.ImageUrl = "~/Images/index.png";
                node.Target = "ChangeRequestType";
                root.Nodes.Add(node);
            }
            this.rtvTreeNode.Nodes.Add(root);

            // Change Grade Codes
            var changeGradeList = this.changeGradeCodeService.GetAll();
            root = new RadTreeNode();
            root.Text = "Change Grade Code";
            root.ImageUrl = "~/Images/index.png";
            root.Value = "-1";
            root.Target = "ChangeGradeCode";
            foreach (var type in changeGradeList)
            {
                RadTreeNode node = new RadTreeNode();
                node.Text = type.Code + ", " + type.Description;
                node.Value = type.ID.ToString();
                node.ImageUrl = "~/Images/index.png";
                node.Target = "ChangeGradeCode";
                root.Nodes.Add(node);
            }
            this.rtvTreeNode.Nodes.Add(root);
        ...
        private void ExportCMDRReport()
        {
            ...
            this.grdDocument.AllowPaging = true;
            this.grdDocument.Rebind();
            var docList = this._PECC2documentService.GetByProjectCode(projectId, UserSession.Current.User.ConfidentialId.GetValueOrDefault(),true).Where(t => ListId.Contains(t.ID)).ToList();
            ...
            var template = docList.Select(t =>
                            new
                            {
                                ...
                                TransInDate = t.IncomingTransId != null? this._PECC2TransmittalService.GetById(t.IncomingTransId.GetValueOrDefault()).IssuedDate: temdate.CreatedDate,
                                TransInDueDate = t.IncomingTransId!= null? this._PECC2TransmittalService.GetById(t.IncomingTransId.GetValueOrDefault()).DueDate: temdate.CreatedDate,
                                TransOutNo = t.OutgoingTransNo,
                                TransOutDate = t.OutgoingTransId!= null? this._PECC2TransmittalService.GetById(t.OutgoingTransId.GetValueOrDefault()).IssuedDate: temdate.CreatedDate,
                            });
