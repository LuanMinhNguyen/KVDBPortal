    ...
    public partial class WR : Page
    {
        ...
        protected void RadAjaxManager1_AjaxRequest(object sender, AjaxRequestEventArgs e)
        {
            ...
            else if (e.Argument.Contains("Cancel"))
            {
                ...
                var obj = this.wrService.GetById(objId);
                if (obj != null)
                {
                    obj.IsCancel = true;
        ...
        protected void grdDocument_OnNeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            ...
            if (ddlProject != null && ddlProject.SelectedItem != null)
            {
                if (this.IsView.Value == "True")
                {
                    ...
                    var objList = new List<WorkRequest>();
                    switch (ddlStatus.SelectedValue)
                    {
                        case "1":
                            objList = this.wrService.GetAll(projectId, txtSearchAllField.Text.Trim());
                            break;
                        case "4":
                            var overDueTask =
                                this.objAssignedUserService.GetAllOverDueTask("Work Request")
                                    .Select(t => t.ObjectID)
                                    .ToList();
                            objList = this.wrService.GetAll(projectId, txtSearchAllField.Text.Trim())
                            ...
                            break;
                        case "5":
                            objList = this.wrService.GetAllCompletedWR(projectId, txtSearchAllField.Text.Trim());
                            break;
                        case "6":
                            objList = this.wrService.GetAllIncompleteWR(projectId, txtSearchAllField.Text.Trim());
                            break;

                        case "7":
                            objList = this.wrService.GetAllCompletedWRWaitingPrint(projectId, txtSearchAllField.Text.Trim());
                            break;
                    }
                    ...
                    this.grdDocument.DataSource = objList.OrderByDescending(t => t.RaisedDate);
                }
                else
                {
                    this.grdDocument.DataSource = new List<WorkRequest>();
                }
            }
        }
        ...
        protected void grdDocument_ItemDataBound(object sender, GridItemEventArgs e)
        {
            if (e.Item is GridDataItem)
            {
                ...
                if (item["PriotiyLevelId"].Text == "1")
                {
                    item["PriorityLevelName"].BackColor = Color.Aqua;
                    item["PriorityLevelName"].BorderColor = Color.Aqua;
                }
                else if (item["PriotiyLevelId"].Text == "2")
                {
                    item["PriorityLevelName"].BackColor = Color.DarkOrange;
                    item["PriorityLevelName"].BorderColor = Color.DarkOrange;
                }
                else if (item["PriotiyLevelId"].Text == "3")
                {
                    item["PriorityLevelName"].BackColor = Color.Red;
                    item["PriorityLevelName"].BorderColor = Color.Red;
                }
            }
        }
