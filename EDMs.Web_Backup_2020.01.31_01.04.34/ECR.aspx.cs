        ...
        protected void grdDocument_OnNeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            ...
            if (ddlProject != null && ddlProject.SelectedItem != null)
            {
                if (this.IsView.Value == "True")
                {
                    ...
                    {
                        case "1":
                            objList = this.ecrService.GetAll(projectId, txtSearchAllField.Text.Trim());
                            break;
                        case "4":
                            var overDueTask =
                                this.objAssignedUserService.GetAllOverDueTask("ECR").Select(t => t.ObjectID).ToList();
                            objList =
                                this.ecrService.GetAll(projectId, txtSearchAllField.Text.Trim())
                                    .Where(t => overDueTask.Contains(t.ID))
                                    .ToList();
                            break;
                        case "5":
                            objList = this.ecrService.GetAllCompletedECR(projectId, txtSearchAllField.Text.Trim());
                            break;
                        case "6":
                            objList = this.ecrService.GetAllIncompleteECR(projectId, txtSearchAllField.Text.Trim());
                            break;

                        case "7":
                            objList = this.ecrService.GetAllCompletedECRWaitingPrint(projectId,
