...
namespace EDMs.Web.Controls.Library
{
    ...
    /// The customer edit form.
    /// </summary>
    public partial class ImportDocumentCode : Page
    {
                ...
                foreach (UploadedFile docFile in this.docuploader.UploadedFiles)
                {
                    ...
                    if (extension == ".xls" || extension == ".xlsx" || extension == ".xlsm")
                    {
                        ...
                        foreach (DataRow prDetailRow in prDetailTable.Rows)
                        {
                            ...
                            var documentCodeObj = this.documentCodeService.GetById(int.Parse(ID));
                            if (documentCodeObj != null)
                            {
                                documentCodeObj.Code = Code;
                                documentCodeObj.Description = Description;
                                documentCodeObj.LastUpdatedBy = UserSession.Current.User.Id;
                                documentCodeObj.LastUpdatedDate = DateTime.Now;
                                documentCodeService.Update(documentCodeObj);
                            }
                            else
                            {
                                documentCodeObj = new DocumentCode();
                                documentCodeObj.Code = Code;
                                documentCodeObj.Description = Description;
                                documentCodeObj.CreatedBy = UserSession.Current.User.Id;
                                documentCodeObj.CreatedDate = DateTime.Now;
                                this.documentCodeService.Insert(documentCodeObj);
                            }
                        }
