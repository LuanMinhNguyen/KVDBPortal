...
namespace EDMs.Web.Controls.Library
{
    ...
    /// The customer edit form.
    /// </summary>
    public partial class ReceivedFromEditForm : Page
    {
        /// <summary>
        /// The folder service.
        /// </summary>
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!this.IsPostBack)
            {
                ...
                if (!string.IsNullOrEmpty(this.Request.QueryString["disId"]))
                {
                    ...
                    var objDis = this.receivedFromService.GetById(Convert.ToInt32(this.Request.QueryString["disId"]));
                    if (objDis != null)
                    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            if (this.Page.IsValid)
            {
                if (!string.IsNullOrEmpty(this.Request.QueryString["disId"]))
                {
                    ...
                    var objDis = this.receivedFromService.GetById(disId);
                    if (objDis != null)
                    {
                        objDis.Name = this.txtName.Text.Trim();
                        objDis.Description = this.txtDescription.Text.Trim();
                        objDis.CategoryId = !string.IsNullOrEmpty(this.ddlCategory.SelectedValue) ? Convert.ToInt32(this.ddlCategory.SelectedValue) : (int?)null;

                        objDis.LastUpdatedBy = UserSession.Current.User.Id;
                        objDis.LastUpdatedDate = DateTime.Now;

                        this.receivedFromService.Update(objDis);
                    }
                }
                else
                {
                    var objDis = new ReceivedFrom()
                    {
                    ...
                    this.receivedFromService.Insert(objDis);
                }
