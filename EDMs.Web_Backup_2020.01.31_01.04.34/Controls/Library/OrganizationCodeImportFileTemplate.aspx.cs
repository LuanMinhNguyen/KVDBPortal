...
namespace EDMs.Web.Controls.Library
{
    ...
    /// </summary>
    public partial class OrganizationCodeImportFileTemplate : Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (UploadedFile docFile in this.docuploader.UploadedFiles)
                {
                    ...
                    if (extension == ".xls" || extension == ".xlsx" || extension == ".xlsm")
                    {
                        ...
                        foreach (DataRow dataRow in datatable.Rows)
                        {
                            ...
                            var Shortform = dataRow["Column3"].ToString();
                            var phone = dataRow["Column4"].ToString();
                            var fax = dataRow["Column5"].ToString();


                            if (!string.IsNullOrEmpty(Code))
                            {
                                var areaobj = new OrganizationCode()
                                {
                                    Code = Code,
                                    Name=Shortform,
                                ...
                                this._OrganizationCodeService.Insert(areaobj);
                            }
                            else
                            {
                                error = "Have Error";
                ...
                if (error.Length != 0)
                {
                    ...
                    this.lblError.Text = "OrganizationCode is empty.</br>";
