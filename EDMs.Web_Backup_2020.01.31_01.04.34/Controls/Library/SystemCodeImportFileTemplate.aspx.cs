...
namespace EDMs.Web.Controls.Library
{
    ...
    /// </summary>
    public partial class SystemCodeImportFileTemplate : Page
    {

        private readonly SystemCodeService systemCodeSevice= new SystemCodeService();
                ...
                foreach (UploadedFile docFile in this.docuploader.UploadedFiles)
                {
                    ...
                    if (extension == ".xls" || extension == ".xlsx" || extension == ".xlsm")
                    {
                        ...
                        foreach (DataRow dataRow in datatable.Rows)
                        {
                            ...
                            var Obj = this.systemCodeSevice.GetByCode(Parent);
                            if (! String.IsNullOrEmpty(DocTypeCode))
                            {
                                var areaobj = new SystemCode()
                                {
                                    Code = DocTypeCode,
                                ...
                                this.systemCodeSevice.Insert(areaobj);
                            }
                            else
                            {
                                error = "Have Error";
