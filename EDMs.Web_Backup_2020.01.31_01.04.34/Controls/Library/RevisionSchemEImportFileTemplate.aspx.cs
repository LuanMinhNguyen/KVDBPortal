...
namespace EDMs.Web.Controls.Library
{
    ...
    /// </summary>
    public partial class RevisionSchemEImportFileTemplate : Page
    {

        private readonly RevisionSchemaService _RevisionSchemeService = new RevisionSchemaService();
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (UploadedFile docFile in this.docuploader.UploadedFiles)
                {
                    ...
                    if (extension == ".xls" || extension == ".xlsx" || extension == ".xlsm")
                    {
                        ...
                        foreach (DataRow dataRow in datatable.Rows)
                        {
                            ...
                            var define = dataRow["column3"].ToString();

                            if (! string .IsNullOrEmpty(Code))
                            {
                                var areaobj = new RevisionSchema()
                                {
                                    Code = Code,
                                ...
                                this._RevisionSchemeService.Insert(areaobj);
                            }
                            else
                            {
                                error = "Have Error";
                ...
                if (error.Length != 0)
                {
                    ...
                    this.lblError.Text = "Scheme Code is empty</br>";
