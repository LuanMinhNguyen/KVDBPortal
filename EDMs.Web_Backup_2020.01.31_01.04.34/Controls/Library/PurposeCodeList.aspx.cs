...
namespace EDMs.Web.Controls.Library
{
    ...
    /// Class customer
    /// </summary>
    public partial class PurposeCodeList : Page
    {
        ...
        private void ExportPurposeCode()
        {
            var filePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"Exports") + @"\";
            //var filePath = Server.MapPath("Exports") + @"\";
            var workbook = new Workbook(filePath + @"Template\CodeTemplate.xlsx");
            ...
            var PurposeList = this.purposecodeService.GetAll().ToList();
            ...
            foreach (var PurposeObj in PurposeList)
            {
                var dataRow = dtFull.NewRow();
                dataRow["ID"] = PurposeObj.ID;
                dataRow["Code"] = PurposeObj.Code;
                dataRow["Description"] = PurposeObj.Description;
            ...
            dataSheet.Cells["C1"].PutValue("Purpose CODE");
            var filename = "PurposeCode.xlsx";
        ...
        {
            var PurposeList = this.purposecodeService.GetAll();
            this.grdDocument.DataSource = PurposeList;
        }
        ...
        protected void grdDocument_DeleteCommand(object sender, GridCommandEventArgs e)
        {
            ...
            this.purposecodeService.Delete(PurposeId);
        ...
        private void LoadScopePanel()
        {
            ...
            if (permissions.Any())
            {
                ...
                foreach (RadPanelItem item in this.radPbScope.Items[0].Items)
                {
                    item.ImageUrl = permissions.FirstOrDefault(t => t.Id == Convert.ToInt32(item.Value)).Menu.Icon;
                    item.NavigateUrl = permissions.FirstOrDefault(t => t.Id == Convert.ToInt32(item.Value)).Menu.Url;
                    if (item.Text == "Transmittal Issue Purpose")
                    {
                        item.Selected = true;
                    }
                }
