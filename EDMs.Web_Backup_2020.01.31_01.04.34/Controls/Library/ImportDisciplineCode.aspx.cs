...
namespace EDMs.Web.Controls.Library
{
    ...
    /// The customer edit form.
    /// </summary>
    public partial class ImportDisciplineCode : Page
    {

        private readonly DisciplineService disciplineCodeService;
        ...
        /// </summary>
        public ImportDisciplineCode()
        {
            this.disciplineCodeService = new DisciplineService();
                ...
                foreach (UploadedFile docFile in this.docuploader.UploadedFiles)
                {
                    ...
                    if (extension == ".xls" || extension == ".xlsx" || extension == ".xlsm")
                    {
                        ...
                        foreach (DataRow prDetailRow in prDetailTable.Rows)
                        {
                            ...
                            var disciplinCodeObj = this.disciplineCodeService.GetById(int.Parse(ID));
                            if (disciplinCodeObj != null)
                            {
                                disciplinCodeObj.Code = Code;
                                disciplinCodeObj.Description = Description;
                                disciplinCodeObj.LastUpdatedBy = UserSession.Current.User.Id;
                                disciplinCodeObj.LastUpdatedDate = DateTime.Now;
                                disciplineCodeService.Update(disciplinCodeObj);
                            }
                            else
                            {
                                disciplinCodeObj = new Discipline();
                                disciplinCodeObj.Code = Code;
                                disciplinCodeObj.Description = Description;
                                disciplinCodeObj.CreatedBy = UserSession.Current.User.Id;
                                disciplinCodeObj.CreatedDate = DateTime.Now;
                                this.disciplineCodeService.Insert(disciplinCodeObj);
                            }
                        }
