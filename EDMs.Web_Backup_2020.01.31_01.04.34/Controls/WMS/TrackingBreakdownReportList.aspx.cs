...
namespace EDMs.Web.Controls.WMS
{
    ...
    /// Class customer
    /// </summary>
    public partial class TrackingBreakdownReportList : Page
    {
        ...
        protected void RadAjaxManager1_AjaxRequest(object sender, AjaxRequestEventArgs e)
        {
            ...
            {
                var breakdownReportId = new Guid(e.Argument.Split('_')[1]);
                var breakdownReportObj = this.trackingBreakdownReportService.GetById(breakdownReportId);
                if (breakdownReportObj != null)
                {
                    breakdownReportObj.IsCancel = true;

                    this.trackingBreakdownReportService.Update(breakdownReportObj);
                    this.grdDocument.Rebind();
                }
        ...
        protected void grdDocument_OnNeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            if (this.ddlProject.SelectedItem!= null)
            {
                ...
                var projectId = Convert.ToInt32(this.ddlProject.SelectedValue);
                var objList = new List<TrackingBreakdownReport>();
                switch (ddlStatus.SelectedValue)
                {
                    case "1":
                        objList = this.trackingBreakdownReportService.GetAllRevTrackingBreakdownReportOfProject(projectId, txtSearchAllField.Text)
                                                    .Where(t => (txtDeadlineFrom.SelectedDate == null || t.PICDeadline >= txtDeadlineFrom.SelectedDate)
                                                            && (txtDeadlineTo.SelectedDate == null || t.PICDeadline < txtDeadlineTo.SelectedDate.Value.AddDays(1)))
                                                    .ToList();
                        break;
                    case "2":
                        objList = this.trackingBreakdownReportService.GetAllTrackingBreakdownReportOfProject(projectId, txtSearchAllField.Text)
                                                    .Where(t => (txtDeadlineFrom.SelectedDate == null || t.PICDeadline >= txtDeadlineFrom.SelectedDate)
                                                            && (txtDeadlineTo.SelectedDate == null || t.PICDeadline < txtDeadlineTo.SelectedDate.Value.AddDays(1)))
                                                    .ToList();
                        break;
                    case "3":
                        objList = this.trackingBreakdownReportService.GetAllComingDueTrackingBreakdownReportOfProject(projectId, txtSearchAllField.Text)
                                                    .Where(t => (txtDeadlineFrom.SelectedDate == null || t.PICDeadline >= txtDeadlineFrom.SelectedDate)
                                                            && (txtDeadlineTo.SelectedDate == null || t.PICDeadline < txtDeadlineTo.SelectedDate.Value.AddDays(1)))
                                                    .ToList();
                        break;
                    case "4":
                        objList = this.trackingBreakdownReportService.GetAllOverDueTrackingBreakdownReportOfProject(projectId, txtSearchAllField.Text)
                                                    .Where(t => (txtDeadlineFrom.SelectedDate == null || t.PICDeadline >= txtDeadlineFrom.SelectedDate)
                                                            && (txtDeadlineTo.SelectedDate == null || t.PICDeadline < txtDeadlineTo.SelectedDate.Value.AddDays(1)))
                                                    .ToList();
                        break;
                    case "5":
                        objList = this.trackingBreakdownReportService.GetAllCompleteTrackingBreakdownReportOfProject(projectId, txtSearchAllField.Text)
                                                    .Where(t => (txtDeadlineFrom.SelectedDate == null || t.PICDeadline >= txtDeadlineFrom.SelectedDate)
                                                            && (txtDeadlineTo.SelectedDate == null || t.PICDeadline < txtDeadlineTo.SelectedDate.Value.AddDays(1)))
                                                    .ToList();
                        break;
                    case "6":
                        objList = this.trackingBreakdownReportService.GetAllInCompleteTrackingBreakdownReportOfProject(projectId, txtSearchAllField.Text)
                                                    .Where(t => (txtDeadlineFrom.SelectedDate == null || t.PICDeadline >= txtDeadlineFrom.SelectedDate)
                                                            && (txtDeadlineTo.SelectedDate == null || t.PICDeadline < txtDeadlineTo.SelectedDate.Value.AddDays(1)))
                                                    .ToList();
                        break;
            ...
            {
                this.grdDocument.DataSource = new List<TrackingBreakdownReport>();
            }
        ...
        protected void grdDocument_DeleteCommand(object sender, GridCommandEventArgs e)
        {
            ...
            this.trackingBreakdownReportService.Delete(objId);
