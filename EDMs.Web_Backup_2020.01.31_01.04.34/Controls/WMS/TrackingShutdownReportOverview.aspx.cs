...
namespace EDMs.Web.Controls.WMS
{
    ...
    /// The customer edit form.
    /// </summary>
    public partial class TrackingShutdownReportOverview : Page
    {
        protected const string ServiceName = "EDMSFolderWatcher";

        private readonly TrackingShutdownReportService srService;

        private readonly TrackingShutdownReportAttachFileService srAttachFileService;
        ...
        /// </summary>
        public TrackingShutdownReportOverview()
        {
            this.srAttachFileService = new TrackingShutdownReportAttachFileService();
            this.srService = new TrackingShutdownReportService();
        ...
        protected void grdDocument_OnNeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            if (Request.QueryString["stausName"] != null && Request.QueryString["projId"] != null)
            {
                ...
                var srListFilter = new List<TrackingShutdownReport>();

                var srList = this.srService.GetAll(projId, string.Empty).Where(t => (fromDate == null || t.CreatedDate >= fromDate)
                                                                                && (toDate == null || t.CreatedDate < toDate.Value.AddDays(1))).ToList();
                var mroverDueTask = this.objAssignedUserService.GetAllOverDueTask("Shutdown Report").Select(t => t.ObjectID).ToList();
                switch (statusName)
                {
                    case "Pending":
                        srListFilter = srList.Where(t => t.IsInWFProcess.GetValueOrDefault()).ToList();
                        break;
                    case "Overdue":
                        srListFilter = srList.Where(t => mroverDueTask.Contains(t.ID)).ToList();
                        break;
                    case "Canceled":
                        srListFilter = srList.Where(t => t.IsCancel.GetValueOrDefault()).ToList();
                        break;
                    case "Completed":
                        srListFilter = srList.Where(t => t.IsWFComplete.GetValueOrDefault() || t.IsCompleteFinal.GetValueOrDefault()).ToList();
                        break;
                }
                    this.grdDocument.DataSource = srListFilter;
            }
        }
                ...
                case "DocDetail":
                    {
                        ...
                        e.DetailTableView.DataSource = this.srAttachFileService.GetByShutdownReport(objId);
                        break;
                    }
            }
        }
