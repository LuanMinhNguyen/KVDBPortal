...
namespace EDMs.Web.Controls.WMS
{
    ...
    /// The customer edit form.
    /// </summary>
    public partial class TrackingProductionMeetingReport : Page
    {
        protected const string ServiceName = "EDMSFolderWatcher";

        private readonly TrackingProductionMeetingService pmService;

        private readonly TrackingProductionMeetingAttachFileService pmAttachFileService;


        /// <summary>
        /// Initializes a new instance of the <see cref="TrackingProductionMeetingReport"/> class.
        /// </summary>
        public TrackingProductionMeetingReport()
        {
            this.pmAttachFileService = new TrackingProductionMeetingAttachFileService();
            this.pmService = new TrackingProductionMeetingService();
        ...
        protected void grdDocument_OnNeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            if(Request.QueryString["stausName"] != null && Request.QueryString["projId"] != null)
            {
                ...
                var pmListFilter = new List<TrackingProductionMeeting>();
                var pmList = this.pmService.GetAllByProject(projId);

                switch (statusName)
                {
                    case "Closed":
                        pmListFilter = pmList.Where(t => t.StatusId == 1).ToList();
                        break;
                    case "Open":
                        pmListFilter = pmList.Where(t => t.StatusId == 2).ToList();
                        break;
                    case "Continous":
                        pmListFilter = pmList.Where(t => t.StatusId == 3).ToList();
                        break;
                    case "Cancel":
                        pmListFilter = pmList.Where(t => t.StatusId == 4).ToList();
                        break;
                }
