...
namespace EDMs.Web.Controls.Document
{
    ...
    /// The customer edit form.
    /// </summary>
    public partial class ContractorTransmittalChangeRequestAttachDocFile : Page
    {
        ...
        /// </summary>
        public ContractorTransmittalChangeRequestAttachDocFile()
        {
                ...
                if (transObj != null)
                {
                  this.RadPane2.Collapsed = transObj.IsSend.GetValueOrDefault();
                   this.radUploadDoc.Enabled=!transObj.IsSend.GetValueOrDefault();
                    ...
                    this.grdChangeRequestDocument.MasterTableView.GetColumn("EditColumn").Visible = !transObj.IsSend.GetValueOrDefault();
                    this.grdChangeRequestDocument.MasterTableView.GetColumn("DeleteColumn").Visible = !transObj.IsSend.GetValueOrDefault();
                }
            }
        }
        ...
        protected void grdDocumentFile_OnNeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            ...
            {
                var transId = new Guid(this.Request.QueryString["objId"]);
                var docFileList = this.contractorTransmittalDocFileService.GetAllByTrans(transId, 1).OrderBy(t => t.FileName);
              ...
              //  DataTable datatable = this.GetMappingFile();
                ...
                foreach (UploadedFile docFile in radUploadDoc.UploadedFiles)
                {
                    ...
                    if (fileNameWithoutExtension.Split('_').Length >= 3)
                    {
                        ...
                        docObj.RefDocId = string.Empty;

                        if (fileNameWithoutExtension.Contains("-DCN-") || fileNameWithoutExtension.Contains("-FCR-"))
                        {
                            var partOfFileName = fileNameWithoutExtension.Split('_')[0].Replace(" ", string.Empty).Split('-');
                            // Get Project Code info
                            ...
                            var changeRequestTypeCode = partOfFileName[1];
                            var changeRequestTypeObj = this.changeRequestTypeService.GetByCode(changeRequestTypeCode);
                            if (changeRequestTypeObj != null)
                            {
                                docObj.ChangeRequestTypeName = changeRequestTypeCode;
                                docObj.ChangeRequestTypeId = changeRequestTypeObj.ID;
                            }
                            // ----------------------------------------------------------------------------------------------
                            ...
                            docObj.DocumentNo = fileNameWithoutExtension.Split('_')[0].Replace(" ", string.Empty);
                            docObj.TypeId = 2;
                        }
                        else
                        {
               ...
               this.transmittalService.Update(transObj);
                ...
                this.grdChangeRequestDocument.Rebind();
            }
        }
        ...
        private bool CheckFileExits(string filename, string docno, string rev)
        {
            ...
            if (objdoc.Count > 0)
            {
                var obj = objdoc.Find(t => t.Revision == rev);
                if(obj!= null)
                {
                    var objAttach = docAttachService.GetAllDocId(obj.ID);
                    if(objAttach!= null)
                    {
                        return objAttach.Find(t => t.FileName == filename) != null ? true : false;
                    }
                    else
                    {
                        return false;
        ...
        protected void grdChangeRequestDocument_OnNeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            if (!string.IsNullOrEmpty(this.Request.QueryString["objId"]))
            {
                var transId = new Guid(this.Request.QueryString["objId"]);
                var docFileList = this.contractorTransmittalDocFileService.GetAllByTrans(transId, 2).OrderBy(t => t.FileName);

                this.grdChangeRequestDocument.DataSource = docFileList;
            }
        }

        protected void grdChangeRequestDocument_OnItemDataBound(object sender, GridItemEventArgs e)
        {
            if (e.Item is GridDataItem)
            {
                var item = e.Item as GridDataItem;
        ...
        protected void grdChangeRequestDocument_OnDeleteCommand(object sender, GridCommandEventArgs e)
        {
            var item = (GridDataItem)e.Item;
            var objId = new Guid(item.GetDataKeyValue("ID").ToString());
            var obj = this.contractorTransmittalDocFileService.GetById(objId);
