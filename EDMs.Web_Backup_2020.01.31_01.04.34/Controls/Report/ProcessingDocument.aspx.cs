...
namespace EDMs.Web.Controls.Report
{
    ...
    public class TaskProcessing
    {
        ...
        public string LeadReview  { get; set; }
        public DateTime ? TransInDate { get; set; }
        public DateTime ? TrasInDueDate { get; set; }
        public string DocumentCode { get; set; }
        public bool IsCtreatOutGoing { get; set; }
        public int DaysOverDue { get; set; }
        public string WFName { get; set; }
        public string WFStep { get; set; }
        public string UserInStep { get; set; }
    }
    ...
    /// Class customer
    /// </summary>
    public partial class ProcessingDocument : Page
    {
        ...
        private readonly ObjectAssignedUserService objectAssignedUserService = new ObjectAssignedUserService();
        ...
        protected void LoadDocuments()
        {
            ...
            DateTime? template =  null;
            overduretask = this.dqreDocumentSevice.GetAllDocInWF().Select(t => new TaskProcessing
            {
                ID=t.ID,
                ...
                TransInDate = t.IncomingTransId != null ?this.dqreTransmittalService.GetById(t.IncomingTransId.GetValueOrDefault()).ReceivedDate:template,
                TrasInDueDate= t.IncomingTransId!= null?( this.dqreTransmittalService.GetById(t.IncomingTransId.GetValueOrDefault()).DueDate!= null ? this.dqreTransmittalService.GetById(t.IncomingTransId.GetValueOrDefault()).DueDate.Value:template ) : template,
                DocumentCode=t.DocumentCodeName,
                WFName=t.CurrentWorkflowName,
                WFStep=t.CurrentWorkflowStepName,
                UserInStep=this.GetUser(t.ID),
            }).ToList();
        ...
        protected string GetUser(Guid objectId)
        {
            var incompleteuser = this.objectAssignedUserService.GetAllIncompleteByDoc(objectId);
            var st = "";
        ...
        private void ExportCMDRDataFile()
        {
            ...
            workbook.Open(filePath + @"Template\DQRE_ReportDocumentsProcessing.xlsm");

            var dataSheet = workbook.Worksheets[0];
            var dtFull = new DataTable();
            var filename = "Documents-Processing-Report_ " + DateTime.Now.ToString("ddmmyy") + ".xlsm";
            ...
            dtFull.Columns.AddRange(new[]
            {
                ...
                new DataColumn("NoIndex", typeof (String)),
                 ...
                 new DataColumn("WFStep", typeof (String)),
    });
            ...
            this.grdDocument.Rebind();
            DateTime? template = null;
            var docList = this.dqreDocumentSevice.GetAllDocInWF().Where(t=>ListId.Contains(t.ID)).Select(t => new {
                ID = t.ID,
                TransmittalNumber = t.IncomingTransNo,
                DocumentNumber = t.DocumentNo,
                DocRev = t.Revision,
            ...
            var disciplineRowCount = 1;
                ...
                dataRow["TrasInDueDate"]= docObj.TrasInDueDate != null ? docObj.TrasInDueDate.Value.ToString("dd/MM/yyyy") : string.Empty;
                 dataRow["DocumentCode"]= docObj.DocumentCode ;
                 dataRow["WFName"]= docObj.WFName ;
                 dataRow["WFStep"]= docObj.WFStep ;
        ...
        private void LoadReportPanel()
        {
            ...
            if (permissions.Any())
            {
                ...
                foreach (RadPanelItem item in this.radPbReport.Items[0].Items)
                {
                    ...
                    if (item.Text == "Documents Processing")
                    {
                        item.Selected = true;
                    }
                }
