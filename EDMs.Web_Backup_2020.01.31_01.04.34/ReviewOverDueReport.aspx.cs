...
namespace EDMs.Web
{
    ...
    /// Class customer
    /// </summary>
    public partial class ReviewOverDueReport : Page
    {
        ...
        protected void RadAjaxManager1_AjaxRequest(object sender, AjaxRequestEventArgs e)
        {
            ...
            {
                this.ExportReviewOverDueStatus();
            }
        }

        private void ExportReviewOverDueStatus()
        {
            ...
            workbook.Open(filePath + @"Template\NPK_ReviewOverDueReport.xlsm");
            ...
            var todoListIncomplete = this.toDoListService.GetAllByProject(projectId)
                                            .Where(t => !t.IsComplete.GetValueOrDefault()
                                                    && (txtFromDate.SelectedDate == null || (t.DeadlineDate != null && t.DeadlineDate.Value >= txtFromDate.SelectedDate.Value))
                                                    && (txtToDate.SelectedDate == null || (t.DeadlineDate != null && t.DeadlineDate.Value < txtToDate.SelectedDate.Value.AddDays(1)))
                                            ).ToList();

            var filename = projectName.Split('-')[0].Trim() + "_" + "Review Over Due_FromDate " + (txtFromDate.SelectedDate != null ?txtFromDate.SelectedDate.GetValueOrDefault().ToString("dd-MM-yy") : string.Empty) + " - ToDate " + (txtToDate.SelectedDate != null ?txtToDate.SelectedDate.GetValueOrDefault().ToString("dd-MM-yy") : string.Empty) + ".xlsm";
            
            if (this.rtvDiscipline.SelectedNode != null)
            {
                todoListIncomplete = todoListIncomplete.Where(t => t.DocDisciplineId == Convert.ToInt32(this.rtvDiscipline.SelectedNode.Value)).ToList();
                filename = Utility.RemoveSpecialCharacterFileName(this.rtvDiscipline.SelectedNode.Text) + "(" + projectName.Split('-')[0].Trim() + ")" + "_" + "Review Over Due_FromDate " + (txtFromDate.SelectedDate != null ?txtFromDate.SelectedDate.Value.ToString("dd-MM-yy") : string.Empty) + " - ToDate " + (txtToDate.SelectedDate != null ?txtToDate.SelectedDate.Value.ToString("dd-MM-yy") : string.Empty) + ".xlsm";
            }
            ...
            dtFull.Columns.AddRange(new[]
            {
                ...
                new DataColumn("DeadlineDate", typeof (DateTime)),
                new DataColumn("AssingFor", typeof (String)),
                new DataColumn("Action", typeof (String)),
            });


            var taskGroupByDisciplineList = todoListIncomplete.GroupBy(t => t.DocDisciplineName);
            foreach (var taskGroupByDiscipline in taskGroupByDisciplineList)
            {
                var disciplineRowCount = 1;
                var taskListOfDiscipline = taskGroupByDiscipline.ToList();
                var dataRow = dtFull.NewRow();
                dataRow["DocNo"] = taskGroupByDiscipline.Key;
                dtFull.Rows.Add(dataRow);


                foreach (var docObj in taskListOfDiscipline)
                {
                    dataRow = dtFull.NewRow();
                    dataRow["DocId"] = docObj.ID;
                    dataRow["NoIndex"] = disciplineRowCount;
                    dataRow["DocNo"] = docObj.DocNumber;
                    dataRow["DocTitle"] = docObj.DocTitle;
                    dataRow["RevName"] = docObj.DocRevName;
                    dataRow["AssingFor"] = docObj.UserName;
                    dataRow["Action"] = docObj.ActionName;
                    if (docObj.DocReceivedDate != null)
                    {
                        dataRow["ReceivedDate"] = docObj.DocReceivedDate;
                    }

                    if (docObj.DeadlineDate != null)
                    {
                        dataRow["DeadlineDate"] = docObj.DeadlineDate;
                    }
        ...
        protected void grdDocument_OnNeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            ...
            this.grdDocument.DataSource = todoListIncomplete;
        }
