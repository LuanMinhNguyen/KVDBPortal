...
namespace EDMs.Web
{
    ...
    /// Class customer
    /// </summary>
    public partial class BreakdownReport : Page
    {
        ...
        private readonly TrackingBreakdownReportService BreakdownReportService = new TrackingBreakdownReportService();
        ...
        protected void grdDocument_OnNeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            ...
            if (ddlProject != null && ddlProject.SelectedItem != null)
            {
                if (this.IsView.Value == "True")
                {
                    ...
                    {
                        case "1":
                            objList = this.BreakdownReportService.GetAll(projectId, txtSearchAllField.Text.Trim());
                            break;
                        case "4":
                            var overDueTask =
                                this.objAssignedUserService.GetAllOverDueTask("Breakdown Report")
                                    .Select(t => t.ObjectID)
                                    .ToList();
                            objList =
                                this.BreakdownReportService.GetAll(projectId, txtSearchAllField.Text.Trim())
                                    .Where(t => overDueTask.Contains(t.ID))
                                    .ToList();
                            break;
                        case "5":
                            objList = this.BreakdownReportService.GetAllCompletedBreakdownReport(projectId,
                                txtSearchAllField.Text.Trim());
                            break;
                        case "6":
                            objList = this.BreakdownReportService.GetAllIncompleteBreakdownReport(projectId,
                                txtSearchAllField.Text.Trim());
                            break;

                        case "7":
                            objList = this.BreakdownReportService.GetAllCompletedBreakdownReportWaitingPrint(projectId,
                                txtSearchAllField.Text.Trim());
                            break;
                    }
